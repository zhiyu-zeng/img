...
clip_date: 2026-07-14T10:40:06+08:00
trace_id: 1bad654e-c193-4e40-b921-40f598711b9a
content_hash: 5b08802fae0c1b3c9c045e75e66752eadf99823a5248b47c9c9cb8ea71eaff0e
status: summarized
tags:
  - 看雪
  - LSPosed检测
...
series: null
feed_source: null
ai_summary: 通过计数 ART 虚拟机中 `ClassLinker` 维护的 `class_loaders_` 链表长度，可以在 LSPosed 和 Shamiko 全面隐藏传统痕迹的情况下，检测到模块注入。
ai_summary_style: key-points
images_status:
  total: 5
  succeeded: 5
  failed_urls: []
notion_page_id: 39d75244-d011-81f3-a405-d1697e7fb6d3
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 通过计数 ART 虚拟机中 `ClassLinker` 维护的 `class_loaders_` 链表长度，可以在 LSPosed 和 Shamiko 全面隐藏传统痕迹的情况下，检测到模块注入。
> 
> - **检测原理：** LSPosed 必须为模块创建 `InMemoryDexClassLoader` 并将其挂在 ART 的 `class_loaders_` 链表上，否则模块会被 GC 回收。该链表是运行时内存数据结构，无法被查询类 API 隐藏。
> - **技术实现：** 通过纯内存指针运算定位链表，不调用任何可能被 Hook 的符号或 API。定位时使用三个运行时特征（VTable 落在 libart、双向链表闭环、节点数合理）进行校验，以适应不同 ART 版本布局。
> - **Android 16 适配：** 解决了原文在 Android 16 上失效的两个问题：1) 清除 ARM64 指针的 Top-Byte 标签以进行正确的内存比较；2) 解析 `/proc/self/maps` 替换失效的 `mincore` 系统调用来探测内存可读性。
> - **真机验证：** 在 Pixel 6 Pro / Android 16 上，干净进程 ClassLoader 计数为 3，LSPosed 注入后计数跃升至 12。所有 Java 层检测（包括 `Class.forName`、堆栈回溯等）均被绕过，唯独此 Native 计数方法成功命中。
> - **局限性与定位：** 该方法依赖宿主 App 自身的 ClassLoader 基线，复杂应用可能需单独校准阈值。最佳实践是将其作为多通道检测中的一环，与匿名内存启发、脏页检测等 Native 手段并联使用，以构建更坚固的对抗体系。

> 配套实现： [Sentry](https://bbs.kanxue.com/elink@bf4K9s2c8@1M7s2y4Q4x3@1q4Q4x3V1k6Q4x3V1k6Y4K9i4c8Z5N6h3u0Q4x3X3g2U0L8$3#2Q4x3V1k6@1j5h3W2K6N6h3W2A6i4K6u0r3M7$3g2F1N6s2u0&6) 的 D7 检测通道（ `class_linker_scan.cpp` ）。本文所有日志与截图均取自 Pixel 6 Pro / Android 16（API 36），LSPosed 模块真机注入。
...
## 一、死局：ClassLinker 的 class_loaders\_ 链表
...
| 概览  | 调试页 |
| --- | --- |
| ![clean overview](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/80ffbf844590d6b0.png) | ![clean debug](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/0887d12389e7bbe5.png) |
...
| 概览  | 调试页 |
| --- | --- |
| ![lsp overview](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/61d8d0a157c74682.png) | ![lsp debug](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/62a823cad5cd9029.png) |
...
> 
> ![mem sig detail](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/254466065e94b86a.png)
...
[#HOOK注入](https://bbs.kanxue.com/forum-161-1-125.htm) [#系统相关](https://bbs.kanxue.com/forum-161-1-126.htm)
...
> **wangzehua · 2 楼**
> 
> ![](https://bbs.kanxue.com/view/img/face/079.gif)
...
> **小黄鸭爱学习 · 8 楼**
> 
> 官方原生rom 直接给我检测到了23个classloader怎么回事。连BL都没解开
