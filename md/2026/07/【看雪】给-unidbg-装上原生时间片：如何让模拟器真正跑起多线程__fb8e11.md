...
clip_date: 2026-07-02T18:06:16+08:00
trace_id: cd800133-5b33-491c-82ef-fc5aece03864
content_hash: 3fe1b32de2181bb12da89fb30c7e8571860643bc660468b7049b88f2a0de9807
status: summarized
tags:
  - 看雪
series: null
feed_source: 看雪·Android安全
ai_summary: 通过在 unidbg 的 Unicorn C 后端添加原生时间片 hook，每 N 条指令自动中断并通知 Java 调度器，解决了协作式调度下 worker 线程无法被正确调度导致 futex 超时的问题。
ai_summary_style: key-points
images_status:
  total: 1
  succeeded: 0
  failed_urls:
    - upload/attach/202607/1072248_ZDGEJ9VAY86SJZY.webp
notion_page_id: 39175244-d011-81ea-a7c6-f8058c39275c
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 通过在 unidbg 的 Unicorn C 后端添加原生时间片 hook，每 N 条指令自动中断并通知 Java 调度器，解决了协作式调度下 worker 线程无法被正确调度导致 futex 超时的问题。
> 
> - **问题根源：** unidbg 的协作式调度模型依赖 safe-point 主动让出 CPU，但 worker 线程从未获得执行机会，导致 futex wait/wake 时间窗口错位，业务逻辑无法推进。
> - **解决方案核心：** 在 Unicorn C 后端实现轻量原生时间片 hook，核心代码约30行，每执行 N 条指令自动停止并返回 TIMESLICE 原因，Java 调度器据此轮转任务。
> - **设计原则：** C 层只负责跑指令和报告停止原因，Java 调度器根据任务状态决定下一个执行者，两层接口简单（枚举值+PC值），避免 JNI 调用带来的性能问题。
> - **实现细节：** 包括停止原因枚举（NORMAL、TIMESLICE 等）、Backend 接口扩展、时间片 hook 代码，以及 Java 桥接层处理时间片耗尽并抛出上下文切换异常。
> - **验证与修复：** 通过分阶段测试验证时间片、调度器轮转和 Futex 语义，并修复任务切换时 TPIDR 和 SP 寄存器污染问题，确保线程上下文正确隔离。
...
![](⚠️ https://bbs.kanxue.com/upload/attach/202607/1072248_ZDGEJ9VAY86SJZY.webp)
