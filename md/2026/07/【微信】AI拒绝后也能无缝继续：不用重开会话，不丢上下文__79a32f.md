...
clip_date: 2026-07-21T08:51:51+08:00
trace_id: a7306095-83ea-4b5b-b858-975647135fc5
content_hash: 7d2dfba3867c67526ea3af7ff73e377b6155b54e266812394939d98fd9e27a45
status: summarized
tags:
  - 微信
  - 安全工具
...
ai_summary: 一个用于清理AI编码工具拒绝记录并注入安全测试上下文的工具，使被中断的会话能无缝继续。
...
> 💡 **AI 总结（key-points）**
>
> 一个用于清理AI编码工具拒绝记录并注入安全测试上下文的工具，使被中断的会话能无缝继续。
> 
> - **支持平台与格式：** 工具支持清理 Codex CLI、Claude Code 和 OpenCode 的会话记录，分别对应 JSONL 和 SQLite 格式。
> - **核心清理功能：** 采用两级策略检测拒绝回复，并通过调用LLM进行智能改写，支持批量处理、推理内容擦除及操作前后对比。
> - **CTF提示词注入：** 可为各平台配置专用的工作空间或Profile，注入安全测试上下文，从源头降低AI拒绝概率。
> - **Web UI管理界面：** 提供统一的会话管理、可视化清理、Diff对比和实时日志功能，支持中英文界面。
