...
clip_date: 2026-07-23T20:57:14+08:00
trace_id: ff854f47-7aa0-4f58-ad2a-746b3e10ed14
...
series: null
feed_source: null
ai_summary: 本文记录了2026平航杯CTF比赛的完整解题过程，重点展示了数字取证、逆向工程及恶意代码分析的技术细节。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3ab75244-d011-81be-ae3b-f042b0c9d0c8
ioc:
  cves: []
  cwes: []
  hashes:
...
> 💡 **AI 总结（key-points）**
>
> 本文记录了2026平航杯CTF比赛的完整解题过程，重点展示了数字取证、逆向工程及恶意代码分析的技术细节。
> 
> - **比赛检材覆盖全面：** 题目涵盖手机、服务器、个人电脑及内存镜像等多种数字检材，解题过程需要综合运用取证、逆向和漏洞分析技能。
> - **数字取证技术贯穿始终：** 解法包括仿真检材、提取数据库信息、分析系统日志（如wtmp）、进行数据恢复以及从内存镜像中提取进程信息和加密密钥。
> - **逆向工程分析加密与混淆：** 对加密APP、.NET木马和JS脚本进行了深入分析，涉及识别加密算法（AES、自定义）、提取硬编码密钥、调试混淆逻辑以及还原恶意代码功能。
> - **恶意样本行为分析复杂：** 揭示了通过API接口进行的投毒攻击、木马的回连C2服务器IP与端口、反沙箱/反调试机制以及在受感染设备上的持久化和传播行为。
> - **技术综合性强：** 案例展示了如何结合静态分析、动态调试和自动化脚本编写，有效应对CTF挑战，体现了团队协作与快速学习能力。
