---
title: 【先知】不猜参数，让目标自己告诉你：基于错误反馈的API参数自动发现
source: https://xz.aliyun.com/news/92561
source_host: xz.aliyun.com
clip_date: 2026-07-29T00:48:47+08:00
trace_id: ee7390fa-7d0d-420b-b47a-ba957a63188f
content_hash: 6aa97b38ab9298a55d6b1c46763108682b8e80ffbd87cafc7ce7c5965e4483a1
status: synced
tags:
  - 先知
  - 漏洞分析
...
series: null
feed_source: null
ai_summary: 利用后端错误反馈可自动化发现 API 参数，通过发送空请求并提取返回消息中的参数名，实现零字典、全自动的接口参数枚举，实测部分接口覆盖率优于传统字典爆破。
ai_summary_style: key-points
images_status:
  total: 1
  succeeded: 1
  failed_urls: []
notion_page_id: 3ab75244-d011-8146-b185-e06fbb1e7bb3
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 利用后端错误反馈可自动化发现 API 参数，通过发送空请求并提取返回消息中的参数名，实现零字典、全自动的接口参数枚举，实测部分接口覆盖率优于传统字典爆破。
> 
> - **核心机制：** 向接口发送缺参请求，解析响应中类似“parameter[x] is missing”等错误信息，直接获取真实参数名，再填入新参数迭代探测，最多3轮。
> - **错误模式匹配：** 引擎内置10组正则，覆盖中英文参数缺失（如“缺少参数”、“Field is required”）、JSON Schema、格式与长度错误等模式，优先解析JSON字段 err_msg 或 message，失败则对原始响应体匹配。
> - **实测对比：** 对 /api/user/modify 发现全部2个参数，而1000词字典仅命中1个；对 /api/auth/login 发现3个参数（含 captcha），字典漏掉；/api/apply/save 只发现1个参数，反映出一次性校验导致的限制。
> - **覆盖局限：** 方法对关闭详细错误、前端拦截校验、参数名混淆或后端合并报错（如仅返回“参数不完整”）的系统无效，但国内大量内网系统常有详细错误反馈。
> - **工程化细节：** 测试值自动匹配（见到 phone 填 13800138000 等），避免因格式错误中断探测，同时硬性限制最多3轮迭代防止死循环。
...
-   **字典爆破** ：准备一个参数名字典（uid、user_id、userId...），逐个试——慢，而且命中率依赖字典质量
...
这些错误信息本质上是 **后端免费送给你的接口文档** 。参数名、是否必填、格式要求——全部写在错误信息里。
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/eabcab2ad86a2d30.png)
...
但 **国内大量高校、政府、企业的内部系统不关详细错误** ——很多运营人员依赖错误信息来调试接口。我在一个月内对四个不同教育机构的测试中，三个的系统都返回了带参数名的错误信息。这个比例说明它不是个例。
