---
title: 【微信】AVL Code 的设计如何避免发生“GPT-5.6 一键清空创业者全盘数据”
source: https://mp.weixin.qq.com/s/BwCWrVfG5_qJ9p4EriVUZQ
source_host: mp.weixin.qq.com
clip_date: 2026-07-25T10:19:21+08:00
trace_id: 96198771-20dc-42c9-810e-d3711d9ba908
content_hash: 731e3883b24a53a84db61e272cae99bf62279f41d11ab4da4469483ea5cc291d
status: summarized
tags:
  - 微信
  - AI应用
...
ai_summary: AVL Code 通过多层确定性约束设计，在系统层面构建“挽具”以避免 AI 编程智能体因概率性失误导致不可挽回的数据损失。
ai_summary_style: key-points
images_status:
  total: 5
  succeeded: 5
  failed_urls: []
notion_page_id: 3a875244-d011-8170-852f-efa2fbc5635f
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> AVL Code 通过多层确定性约束设计，在系统层面构建“挽具”以避免 AI 编程智能体因概率性失误导致不可挽回的数据损失。
> 
> - **行为可见性**：所有工具调用记录为独立卡片，状态明确，会话记录持久化并支持轨迹回放，无需用户实时监控。
> - **动作可控性**：工具配置启用、询问、禁用三态开关，可按工作模式自定义拦截规则；审批通过多端推送，超时默认拒绝。
> - **边界先划清**：工作区路径校验禁止访问敏感目录；网络边界仅放行 http(s) 协议，拦截内网和云元数据地址。
> - **红线机制**：即使开启无限制模式，samples/沙箱禁止执行、破坏性命令参数校验等硬底线依然生效，不受开关影响。

**安天集团** *2026年7月25日 10:00*

本文原载于安天AVL Code编程智能体技术blog，发布日期为2026-07-12，原文地址为：https://www.avlcode.cn/blog/agent-safety-gates/

![AVL Code 的设计如何避免发生“GPT-5.6 一键清空创业者全盘数据”](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/a68b74539afdbf77.jpg)
...
事情的经过，X 用户 @AYi_AInotes 在当天上午给出了详细拆解：Matt 在自己的 Mac 上为本地 Agent 开启了 Full Access 全权限，让一个子代理执行例行的文件清理——同类任务此前已经运行过几百次。这一次，GPT-5.6-Sol 未能正确展开 `$HOME` 变量，实际执行的命令成了 `rm -rf /Users/mattsdevbox` 。等 Matt 发觉异常、手动终止进程时，多年积累的代码、文件、照片已经损失大半。事后 Agent 生成了一份事故报告，承认路径展开错误。报告写得诚恳，文件无法恢复。
...
![AVL Code 子代理任务详情截图：系统指令、任务输入与逐条工具调用，含一条被拒绝的调用](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/a18892e8f16a0344.jpg)

AVL Code 子代理任务详情截图：系统指令、任务输入与逐条工具调用，含一条被拒绝的调用
...
![AVL Code 工具配置界面截图：每个工具的启用/询问/禁用三态开关，以及 fs.exec 的放行与拦截规则编辑面板](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/781fd0a671ad8858.jpg)

AVL Code 工具配置界面截图：每个工具的启用/询问/禁用三态开关，以及 fs.exec 的放行与拦截规则编辑面板
...
![AVL Code 无限制模式风险确认弹窗截图：列明放开的权限与保留的底线拦截，确认按钮带倒计时](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/b5c9868c266036e7.jpg)

AVL Code 无限制模式风险确认弹窗截图：列明放开的权限与保留的底线拦截，确认按钮带倒计时
...
@AYi_AInotes 在事故拆解推文的结尾给出四条教训，前三条我们完全同意；第四条——「模型厂商的安全底线不一样」，Matt 也表示此事让他「一千倍地更信任」另一家的模型——我们只同意一半。模型的谨慎值得作为选型依据，但不值得作为安全依据：再谨慎的模型也是概率性的，把关键数据托付给概率，与 Full Access 常开没有本质区别。
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/d368287ad1f83d53.jpg)
