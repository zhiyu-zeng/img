...
clip_date: 2026-07-21T14:56:34+08:00
trace_id: 2bf29b5f-2e7a-42ba-acdf-3b3e8dcd304f
content_hash: bdf31579446983b09a7bbdf42b1fe0c155188d450b01ea8b407717765b75fc1f
status: summarized
tags:
  - 微信
  - AI应用
...
series: null
feed_source: null
ai_summary: AI智能体正通过工具集成改变渗透测试模式，它能降低重复性体力劳动，但核心判断与风险控制仍需人类专家把握。
ai_summary_style: key-points
images_status:
  total: 4
  succeeded: 4
  failed_urls: []
notion_page_id: 3a475244-d011-8179-af67-c1614cc2590d
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> AI智能体正通过工具集成改变渗透测试模式，它能降低重复性体力劳动，但核心判断与风险控制仍需人类专家把握。
> 
> - **工具介绍：** 开源插件 Burp_Hermes 将 AI 智能体 Hermes 集成到 BurpSuite 中，主要功能是**自动化分析数据包和处理 Payload**，用户可通过右键菜单将抓取的流量发送给 AI 进行分析。
> - **行业共识：** 绝大多数安全从业者将 AI 视为**赋能工具而非替代者**，82%的白帽已在工作中使用 AI，但仅 12% 认为 AI 会取代自己。AI 善于节省成本和时间，但在**复杂业务逻辑漏洞、攻击链组合及上下文判断**方面，人类专家仍具优势。
> - **性能现状：** 当前 AI 在真实漏洞利用场景中**自主成功率较低**，如自主利用真实 CVE 的成功率仅为 13%–21%；在标准测试中准确率高的模型，应用于生产环境时准确率会**大幅下降**。
> - **部署方式：** 该插件需先在目标环境（如 Kali）部署 Hermes 服务，然后在 Burp 中配置 Jython 环境并加载插件，**使用门槛相对较低**。
> - **使用边界：** 作者强调使用此类 AI 工具必须遵守三条红线：**严守授权范围、人类保留最终判断权、防止自身核心技能退化**。工程实践上，正通过“LLM-as-judge”裁判模型和流量网关等机制来约束 AI 自主行为，确保安全。
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/926b17258964be37.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/0029f5848e32814a.png)

第二步，加载插件。Burp 里 `Extensions` → 添加，选插件文件，装完在 `Output` 能看到成功日志。

![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/595c70232a4895d4.png)

用起来就一句话：常规抓包后，选中目标URL，右键 `Extensions` → `Hermes` ，AI接管这段流量。分析完，直接在 Hermes 里做下一步操作。

门槛不高，有个Kali和会装扩展就能跑。

![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/53fb6fdb648bcc1d.png)
