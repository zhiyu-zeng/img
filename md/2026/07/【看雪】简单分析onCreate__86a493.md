...
clip_date: 2026-07-14T10:42:34+08:00
trace_id: 625e05b5-41c9-4352-95a8-f8ed41067f98
content_hash: 9e19d781ec9e5b3d8659e88d8f6a19053e333bd2959a4a38fa948ff6942f32be
status: summarized
tags:
  - 看雪
  - Android逆向工程
...
series: null
feed_source: null
ai_summary: 百度加固Dex VMP通过自定义解释器保护onCreate方法，其入口被替换为对native函数V的调用，并借助虚拟寄存器和堆栈执行自定义字节码。
ai_summary_style: key-points
images_status:
  total: 14
  succeeded: 14
  failed_urls: []
notion_page_id: 39d75244-d011-81cf-b29f-e46387c3d388
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 百度加固Dex VMP通过自定义解释器保护onCreate方法，其入口被替换为对native函数V的调用，并借助虚拟寄存器和堆栈执行自定义字节码。
> 
> - **入口与跳板函数：** `MainActivity.onCreate`被替换为调用`A.V(0xab000000, this, args)`，V是位于`libbaiduprotect.so`中的native方法，作为VMP的起始入口。
> - **关键数据结构：** 入口参数`0xab000000`用于索引`vmState`结构体，其中包含方法信息表；分析表明虚拟寄存器的数量由方法信息(`*(v10 + 18)`)决定并分配内存。
> - **解释器核心：** 最终执行函数内部通过`BR X8`跳转，其地址来自`off_BD040`处的函数指针表（约256个），X26寄存器被推测为指向虚拟字节码的指针。
> - **指令执行跟踪：** 通过trace可分析具体指令，如`0xf9`对应`const-string`，`0x6e`对应`invoke-super`调用`super.onCreate`，`0x70`对应`check-cast`等，展示了虚拟指令与Dalvik字节码的对应关系。
> - **分析辅助工具：** 作者使用了乐子人大的trace工具记录执行流，并利用IDA的hrtng插件去除了不透明谓词（如`((dword_C0118 * (dword_C0118 - 1)) & 1)!= 0`）以简化分析。
...
![image-20260612195144947](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/66c069c0ae69bdbe.png)
...
![image-20260612205053937](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/3a717c6ba6f46174.png)
...
![image-20260612211521756](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/2a3b4ad643a2067b.png)
...
![image-20260612212430951](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/c0e6718f8e78e397.png)

然后又取了2字节，并且取了高8位

![image-20260612213238667](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/d30784c26e89f8f3.png)

然后写了个x0到对应的地址，w19是索引，是不是可以合理推测一下，x14就是虚拟寄存器的基地址，所以标出来，然后x0是什么？

发现是是NewGlobalRef的返回结果

![image-20260612213901117](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/895b35385f07b516.png)

![image-20260612220525497](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/5ed9e008a7ec9c0c.png)
...
![image-20260612221635745](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/eec8af25144eb65f.png)
...
![image-20260612230455773](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/fb8437ed369a78e8.png)
...
![image-20260612231337352](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/01d5e45a11bc2842.png)
...
![image-20260613120613279](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/ee16c6bdc912d280.png)
...
![image-20260613161244813](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/ed034f7805b22984.png)
...
![image-20260613163426290](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/a0ff501f99fb9569.png)
...
![image-20260613170750705](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/58d783dd249fa8e9.png)
...
- [baidu.zip](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/attach/2026/07/88be0b4b502ce24c.zip) （1.24MB，21次下载）
