---
title: 【先知】FastJson2 Hash 碰撞 RCE 分析与复现
source: https://xz.aliyun.com/news/92608
source_host: xz.aliyun.com
clip_date: 2026-07-30T14:11:33+08:00
trace_id: c6e96a52-cc85-4b44-9a9e-22921f7f58e6
content_hash: d8c0e15725fc6e56d836eaa9fe0c33e3f98e00e6800f806a3fe8051188953bdd
status: synced
tags:
  - 先知
  - FastJson2
...
ai_summary: FastJson2 2.0.53 的 checkAutoType 方法依赖 FNV‑1a 哈希比对，使用约束求解器即可生成碰撞类名，配合 SpringBoot URLClassLoader 实现远程代码执行。
ai_summary_style: key-points
images_status:
  total: 29
  succeeded: 29
  failed_urls: []
notion_page_id: 3ad75244-d011-81d9-a3cf-cd3616d4b029
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> FastJson2 2.0.53 的 checkAutoType 方法依赖 FNV‑1a 哈希比对，使用约束求解器即可生成碰撞类名，配合 SpringBoot URLClassLoader 实现远程代码执行。
> 
> - **漏洞触发点：** JSON.parseObject(可控, Object.class) 等接口在反序列化时，即使未开启 autotype，也会进入 ObjectReaderImplObject.readObject，进而调用 checkAutoType 进行哈希白名单校验。
> - **哈希碰撞原理：** checkAutoType 使用 FNV‑1a‑64 位算法计算传入类名的哈希值并与硬编码哈希比较；通过 z3 约束求解器可为任意前缀生成冲突后缀，使任意类名通过校验并触发 loadClass。
> - **Payload 构造：** 将远程恶意 jar 的类名（如 “jar:http:..IP十进制:端口.文件名!.类名”）作为前缀，生成碰撞后完整字符串，放入 JSON 的 “@type” 字段，即可加载远程类。
> - **利用环境与触发方式：** 目标需使用 SpringBoot 类加载器，且 jar 文件可被远程下载；除了 parseObject(可控, Object.class)，还可利用 List、Set、Comparable 等接口触发同一调用链。
> - **攻击链扩展：** 攻击者可先生成内存马 jar，通过上述方式加载后驻留在 /proc/self/fd 目录，再用碰撞类名实现二次注入，完成持久化控制。
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/7771053964d095bc.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/41bf4e559592fe8c.png)

最终我们可以看到字节码信息:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/f83b8003443567da.png)

这里说明一下实验时失败的尝试（ASM 输出并没有携带行号信息导致无法 Debug）, 首先将导出出来的字节码符合包名结构, 保存到 jar 中:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/c08b07469eed2963.png)

随后增加 classpath, 并且将其置顶:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/8bd34a7c5083bcf5.png) 但是由于该字节码由 ASM 生成, 导致不存在行号信息, 可以安装: [https://github.com/Col-E/Recaf/releases/tag/4.0.0-alpha](https://github.com/Col-E/Recaf/releases/tag/4.0.0-alpha) 中的recaf-launcher-gui-0.8.8.jar来进行修复行号, 需要注意的是首次运行需要要求安装依赖库（javaFX 等）, 使用 proxychains4 运行该 jar 进行安装可加快速度. 但在实际场景中发现反编译存在错误信息:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/5033ef47db3849ca.png)

正常会调用该字节码的 readObject 方法, 并且整个 ASM 中不存在反射的逻辑:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/3076ac65724ab11d.png)

但部分方法会调用 checkAutoType:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/27a1749590c70ac0.png)

## RCE 场景

参考: [https://mp.weixin.qq.com/s/4jl2kv_JRSDUAUZyc1jw5A](https://mp.weixin.qq.com/s/4jl2kv_JRSDUAUZyc1jw5A), 官网的 commit 记录中存在对 payload 的测试记录:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/e8c387fbcbbd1c78.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/d7d3a45c27f57ebf.png)

在ObjectReaderImplObject::readObject反序列化流程中会判断是否开启了 checkAutoType:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/8274fecf88994881.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/261a04ba7c4c5520.png)

而后面的逻辑存在一个黑白名单校验的缺陷:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/c175739e32070f71.png) 即使没有开启 autotype 功能, 同样会进入 hash 比较的逻辑, 那么如果这里的 hash 能够被暴力破解或其他手段猜测出来（由于这里 Hash 值的判断是根据结果进行判断，而过程中不同的字符参与异或|乘法运算是会存在冲突的结果的）, 那么则会进入 loadClass 逻辑:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/4cf44c4ba71b7229.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/42dbbdb7d07a1d72.png)

最终对应 fastjson 场景解密 -6293031534589903644 效果:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/b79f493f8bb81b28.png)

通过解方程的形式成功达到 Hash 碰撞的效果, 那么在此基础之上我们只需要将我们在 fastjson 1.2.83 中的 payload 作为前缀即可. 以任意字符为前缀的话, 对于 fastjson 的计算来说仅仅是起点不同了:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/acbed9c9816a30f2.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/3c244997dccb34a2.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/f443846e78efd615.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/ed65a8b43a98da6d.png)

当前仅是驻留到受害机 /proc/{pid}/fd 中案例, 先使用 0kb 的文件做测试:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/0de26fc329a9e835.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/478d160810e7751b.png)

随后是工具生成的 result.txt 进行爆破:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/f65f49499cde90ad.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/da194fe2c8aa8c9d.png)

最终结果:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/1b1d0ca38cf94c89.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/00330e26d61ef7a3.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/7f3cc42385c22a65.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/5c30ce5805dc460e.png)

相对于 JSON.parse 来说, 多了一步选择器的操作:

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/07/bcf4003c5642f4c0.png)
