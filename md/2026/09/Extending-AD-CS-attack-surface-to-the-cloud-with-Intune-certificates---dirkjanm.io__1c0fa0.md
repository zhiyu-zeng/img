---
title: Extending AD CS attack surface to the cloud with Intune certificates - dirkjanm.io
source: https://dirkjanm.io/extending-ad-cs-attack-surface-intune-certs/
source_host: dirkjanm.io
clip_date: 2026-09-25T10:17:09+08:00
trace_id: 20ec1126-cd73-4e6c-af19-84e2c737c3e2
content_hash: e86dae5ef75adb167cf822fcb892a0f99c64764b8114ae93fc8ca031840d31ff
status: synced
tags:
  - 漏洞分析
  - Intune证书攻击
...
ai_summary: 通过 Intune 证书连接器与可伪造 SAN 的 AD CS 模板，攻击者可从云端获取域控证书并申请 TGT，最终拿下本地域。
ai_summary_style: key-points
images_status:
  total: 17
  succeeded: 17
  failed_urls: []
notion_page_id: 3e675244-d011-81e4-840d-c29b03633181
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 通过 Intune 证书连接器与可伪造 SAN 的 AD CS 模板，攻击者可从云端获取域控证书并申请 TGT，最终拿下本地域。
> 
> - **攻击前提：** AD CS 模板须设置 `EnrolleeSuppliesSubject`（Intune 模板的硬性要求）、支持客户端认证，且 NDES 服务账号或连接器主机有权注册；CA 需位于 `NTAuthCertificates`。
> - **两条路径差异：** PKCS 由 Intune 生成私钥与 CSR，设备只与 Intune 通信，可为连接器可访问的任意模板注册；SCEP 私钥不出设备，设备经代理直连 NDES，但 CSR 会先送 Intune 校验，注册模板固定在 NDES 注册表中。
> - **管理员利用：** 有 Intune/全局管理员权限即可新建配置配置文件，把 DNS SAN 指向域控主机名，并硬编码 `URL=tag:microsoft.com,2022-09-14:sid:<SID>` 满足强映射；约 5–10 分钟同步后证书入计算机存储，用 Rubeus/PKINITtools 以指纹申请 TGT；PKCS 还能从 SyncML 的 `PFXCertInstall` 消息中抓取 PFX 及其加密口令。
> - **普通用户 ESC1：** 若 SAN 使用 `{{DeviceName}}`、`{{IMEI}}`、`{{SerialNumber}}` 等可伪造变量，且域名后缀与本地 AD 一致，低权用户改名即可。强映射强制（2025 年 9 月起唯一支持模式）时仅 SCEP 可行——自制 CSR 中可加入任意 SID 安全扩展并通过校验。
> - **缓解：** 证书配置中禁用可伪造标识符；对域管/域控签发证书或据此申请 TGT 的异常行为告警；Autopilot 硬件 ID 注册可阻断伪造设备注册，但并非其设计目的。
...
![Intune PKCS flow diagram](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9a36870fccef64b9.png)
...
![Intune SCEP flow diagram](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/be8275217e3fb5ca.png)
...
![Intune PKCS configuration profile](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a5db50dcc4dd50d7.png)
...
![Intune PKCS configuration profile](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/39a3363c39cb0783.png)
...
![Intune profile scope](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/fb7bae0f89c522c9.png)
...
![Triggering a manual sync](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/b17722919d53b7fa.png)

![Certificate in the computer certificate store](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/8f1399f5a7a05470.png)

![Certificate SANs mapping to a DC](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/63006065e487f23d.png)
...
![Requesting a TGT with Rubeus](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/73573166fc23d663.png)
...
![PFXCertInstall in the SyncML](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/f18d67c2eb9a3aea.png)
...
![PFXCertInstall in the SyncML](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/992552e0b40e38f7.png)
...
![Warning against using these variables in the Microsoft documentation](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9747fce6e4865314.png)
...
![Vulnerable SCEP template](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9852a1f45c22beb3.png)

Let’s rename our device to match a Domain Controller name:

![Vulnerable SCEP template](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/d292635ca972e8b7.png)
...
![SCEP enrollment data](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/0f29d57af1f263ed.png)
...
![SCEP enrollment success](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a987ba2cec4b7d9e.png)
...
![Requesting a TGT with PKINITtools](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/bee72c9628653f0d.png)
