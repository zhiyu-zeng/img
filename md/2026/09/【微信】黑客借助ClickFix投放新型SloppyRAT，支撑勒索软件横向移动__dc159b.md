---
title: 【微信】黑客借助ClickFix投放新型SloppyRAT，支撑勒索软件横向移动
source: https://mp.weixin.qq.com/s/7OZXhNzlSR1eoAJa0qDD2A
source_host: mp.weixin.qq.com
clip_date: 2026-09-11T16:35:04+08:00
trace_id: e0d7fcf2-c465-4a58-acb9-b388c3e0ccda
content_hash: ca9490f293eb4bc1d03d47329b0936d9c14d2006b36f2cff46082856a644cfad
status: synced
tags:
  - 微信
  - 恶意样本
...
ai_summary: SloppyRAT 是 2026 年 6 月出现的新型 Windows 远控木马，经 ClickFix 多阶段攻击链投递，为勒索软件团伙提供入侵后侦察、反向代理与横向移动能力。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3d875244-d011-81db-9ecd-efc5ac8a867b
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> SloppyRAT 是 2026 年 6 月出现的新型 Windows 远控木马，经 ClickFix 多阶段攻击链投递，为勒索软件团伙提供入侵后侦察、反向代理与横向移动能力。
> 
> - **投递链：** 调用系统自带 finger.exe 从攻击者服务器下载批处理脚本（TCP 79 端口现代企业几乎不用，出现即高价值告警）；脚本再把 curl.exe 复制到 AppData、改成数字后缀 .com 文件，用它从 GitHub 拉取 IronPython 运行环境。
> - **后续组件：** 以特征 User-Agent K8VGmQTrzX 从 skipraid[.]com 下载 CastleLoader 与 CastleRAT；最后从 Azure Blob 取 config.py，借 hostfxr.dll 反射加载 SloppyRAT（特征 UA 为 DLLMemLoader/1.0）。
> - **核心能力：** 采集主机与安全软件信息，枚举进程、服务、用户、文件、注册表、环境变量与网络连接，支持下载文件、WMI 查询、篡改 Defender 配置；最关键的是由 /api/poll 下发的反向 SOCKS 代理，可把一台普通权限主机变成打入内网的跳板。
> - **规避手段：** 用 C++ 原生实现 47 条类 PowerShell 命令（whoami、Get-Process、Test-NetConnection 等映射 Windows API），必要时才加载 .NET CLR 在进程内调用；cmd 模式走 Win32_Process::Create；失败时启动 powershell.exe 并把父进程伪造成 explorer.exe。
> - **C2 与缺陷：** HTTPS 通信比对服务器 TLS 证书硬编码 SHA-256 哈希实现证书锁定，另有经 Polygon JSON-RPC 解析 C2 的 EtherHiding 备选方案；但 Run 注册表自启缺少完整 DLL 路径与导出函数调用，持久化存在缺陷，说明家族仍在迭代。

**安全圈的那点事儿** *2026年9月11日 15:39*
...
木马的 cmd 模式通过WMI接口 Win32_Process::Create 执行命令，进一步让攻击者行为混在正常管理员运维活动中，难以区分。  
