...
clip_date: 2026-09-07T13:17:42+08:00
trace_id: d3d72b5d-b83f-4f72-ad4d-6aeec74e77c8
content_hash: 501fd37cbfa096e2e399801de1e5b683ce75b61e9b2a5f95396f109d87f81816
status: synced
tags:
  - Android逆向
  - Frida
...
ai_summary: 针对新版梆梆加固的 libDexHelper.so，逆向其内置 loader 并静态解密出真正 so，再通过 hook mmap 定位动态基址、patch 关键函数和 kill 类 svc 调用，绕过 frida 检测。
ai_summary_style: key-points
images_status:
  total: 3
  succeeded: 3
  failed_urls: []
notion_page_id: 3d475244-d011-812f-9325-f8b699df1742
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 针对新版梆梆加固的 libDexHelper.so，逆向其内置 loader 并静态解密出真正 so，再通过 hook mmap 定位动态基址、patch 关键函数和 kill 类 svc 调用，绕过 frida 检测。
> 
> - **壳加载链路：** 外层 so 入口 sub_4780 以 ELF 魔数 0x464C457F 扫描自身基址，sub_33F8 是主流程，实际实现了一个 mini linker/loader：解析 /proc/self/maps 定位自身路径，解密内层 ELF、映射 PT_LOAD、dlopen 依赖并完成重定位。
> - **静态解密关键参数：** 内层 ELF 头从偏移 0x8000 读 64 字节，使用偏移 0x1181DD 处 20 字节中的 16 字节 key 做类 RC4 解密；隐藏 payload 其它内容从头部之后开始按单字节 0x41（尾部 key 的第 17 字节）XOR，解密后校验 ELF 魔数 7F 45 4C 46。
> - **验证/替代 dump 方式：** 该 so 加载流程与 yuuki/soLoader 项目接近，不必强行静态还原；可在 dlopen onLeave 时扫描匿名可执行内存动态 dump 内层 so。
> - **定位真实基址：** 内层 so 被映射到自己申请的匿名内存，hook libc 的 mmap，按来源属于外层 addr、len=0x134000、prot=3、flags=0x22、fd=-1 过滤返回值，即可得到 hook JNI_OnLoad 等函数的基址。
> - **反 frida 检测绕过：** 用 stalker_trace_so 定位到可疑入口后，实测杀进程来自混淆后的 JNI_OnLoad 链路中的 svc kill(129) 直接退出；把 sub_1304C replace 为 return 0，并 patch 触发点 0x190a0/0x19420 即可运行，也可用 IDA 脚本回溯 x8 系统调用号批量生成对全部退出类 svc 的 patch。
> 
> 
> OUTPUT_PATCH: []
...
![image-20260408224251994](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/7a5aa76890c44f5a.webp)
...
![image-20260412213539531](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/2631b9b791bc8d4d.webp)

![image-20260412213651042](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/fba5a5ee93d79986.webp)
