...
source: https://blog.orange.tw/posts/2025-01-worstfit-unveiling-hidden-transformers-in-windows-ansi/
source_host: blog.orange.tw
clip_date: 2026-09-08T23:49:03+08:00
trace_id: 0b3151e5-51ac-482c-b606-396492477f4b
content_hash: e8ef665dc6ab30a396e3c58983241338371f1ae9276817f2bcdf526c2f71dfc2
status: synced
tags:
  - Windows逆向
  - 漏洞分析
...
ai_summary: WorstFit 攻击利用 Windows 将 Unicode 转为 ANSI 时的 Best-Fit 字符映射，把全角引号、¥、₩ 等字符变成标准引号、斜杠或反斜杠，进而在系统层面触发路径穿越、参数注入乃至远程代码执行，波及 PHP、curl、tar、Excel 等多款知名软件。
ai_summary_style: key-points
images_status:
  total: 18
  succeeded: 18
  failed_urls: []
notion_page_id: 3d575244-d011-8186-8ee9-fae37c99ce3f
ioc:
  cves:
    - CVE-2024-45720
    - CVE-2024-4577
...
> 💡 **AI 总结（key-points）**
>
> WorstFit 攻击利用 Windows 将 Unicode 转为 ANSI 时的 Best-Fit 字符映射，把全角引号、¥、₩ 等字符变成标准引号、斜杠或反斜杠，进而在系统层面触发路径穿越、参数注入乃至远程代码执行，波及 PHP、curl、tar、Excel 等多款知名软件。
> 
> - **转换机制：** Windows ANSI API 会把 UTF-16 字符串经 Best-Fit 近似映射到代码页字符；同一字符在不同代码页映射不同，如 U+FF02 全角引号在 125x/874 下变普通双引号，U+00A5 日元符在日文代码页变反斜杠，这使黑名单类过滤可被绕过。
> - **三大攻击技术：** 一是 Filename Smuggling，用映射为 `/` 或 `\` 的字符污染路径处理；二是 Argument Splitting，通过 `GetCommandLineA`/`int main()` 的 ANSI 转换改变命令行语法，注入额外参数；三是 Environment Variable Confusion，利用 `getenv` 等返回 Best-Fit 结果绕过环境变量安全限制。
> - **真实漏洞实例：** CVE-2024-4577 只需 `?%ADs` 即可绕过 PHP-CGI 参数检查实现 RCE；CVE-2024-49026 通过恶意文件名的 Excel 文件配合“打开方式”实现点击即 RCE；ElFinder 借 Windows 自带 `tar.exe` 的参数分割漏洞弹出 calc。
> - **影响面与修复困难：** curl、openssl、Perl 等多个项目普遍受影响，但因问题源于 CRT `mainCRTStartup` 自动调用 ANSI API 与系统转换层，修复需引入 `wmain` 和宽字符改写；多数厂商以“Windows 功能”为由 Wont Fix，MSRC 仅部分受理。
> - **缓解建议：** 用户可启用 Windows 的 UTF-8 Beta 选项；开发者应尽量使用 Wide Character API（如 `_wgetcwd`、`_wgetenv`、`GetCommandLineW`），避免使用会触发 ANSI 转换的非宽字符接口。

![preview](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/4068181b0f20c1c4.jpg)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/b27f0b9abce87ce7.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/75359acf2e62f98c.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/735f3375a7624741.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/cb78c1ac094ae1e5.png)  
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/02c0bb9eaea08721.png)  
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/198555eaee41b354.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/0e41435967917b41.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/2a3fe7b1ab120a8a.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/bea1173a2f40d295.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/7e0ee7597ca55816.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/211c4e554d03c476.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/35fd1d079b62dc5e.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9ca3713c7d91d960.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/ed9a2e9851e004c0.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/6a49428cb9ffddb6.png)

*↓ World Map of WorstFit:*

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/fd66dc9cc347774b.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/88fd6f5af3a6b9df.png)
