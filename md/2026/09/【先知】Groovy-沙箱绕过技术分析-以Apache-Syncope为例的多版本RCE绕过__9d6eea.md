---
title: 【先知】Groovy 沙箱绕过技术分析-以Apache Syncope为例的多版本RCE绕过
source: https://xz.aliyun.com/news/92835
source_host: xz.aliyun.com
clip_date: 2026-09-15T18:00:33+08:00
trace_id: aebc4ea6-b5b8-4a4e-b1b1-37de6095c703
content_hash: c1d719c1ca27932a4e6c7fd9fd3947b2820ef7de1a0bcebceb2c65335797b0d8
status: synced
tags:
  - 先知
  - 漏洞分析
...
ai_summary: Groovy 黑名单沙箱靠编译期织入检查点，可被解释器逃逸与 InvokerHelper 间接调用两类手法绕过，Apache Syncope 由此出现多版本 RCE。
ai_summary_style: key-points
images_status:
  total: 4
  succeeded: 4
  failed_urls: []
notion_page_id: 3dc75244-d011-819e-8372-f6b1e52f64d4
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> Groovy 黑名单沙箱靠编译期织入检查点，可被解释器逃逸与 InvokerHelper 间接调用两类手法绕过，Apache Syncope 由此出现多版本 RCE。
> 
> - **沙箱机制：** SandboxTransformer 在编译期把调用改写为 Checker.checked* 跳板，运行期 GroovyInterceptor 链按"new/method/staticMethod + 类名 + 方法签名"静态比对黑名单。
> - **三个前提：** 脚本必须经它编译、调用以 Groovy 语法直接书写、签名能查到目标；三类绕过分别打破其中一条。
> - **攻击入口：** `POST /syncope/rest/connectors/check` 的 testScript 由 ConnId 的 GroovyScriptExecutor 执行，仅需 CONNECTOR_READ 权限。
> - **解释器逃逸：** `new GroovyShell().evaluate(...)`、GroovyClassLoader、Eval、ScriptEngineManager 四个入口，因 ConnId 黑名单比 Syncope 少约 100 条签名，单请求即可 RCE。
> - **间接调用逃逸：** `InvokerHelper.invokeStaticMethod` 把真实目标藏进运行期字符串参数，派发下沉到 Java 反射层，静态签名匹配在此路径上是盲的，黑名单无法闭合。
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/2480473e7dbdc915.png)

第二步，触发 reload，新 JAR 被直接加载，无签名校验、无白名单：

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/73a500550ad263a5.png)

第三步，触发恶意连接器的 `test()` ， `id` 执行成功，输出通过异常消息返回：

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/08536bf055f00585.png)
...
沙箱的检查点是编译期织入的，那么只要能在脚本里创建一个 **新的、使用默认配置的 Groovy 解释器**，把真正的 payload 作为字符串交给它，内层代码就会在一个从未织入检查的环境中编译执行。这不是绕过检查，而是让检查的调用点根本不存在。
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/cf2c6a6e93f158f8.png)
