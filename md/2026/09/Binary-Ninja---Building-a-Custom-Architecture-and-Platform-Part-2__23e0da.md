---
title: "Binary Ninja - Building a Custom Architecture and Platform: Part 2"
source: https://binary.ninja/2026/02/26/quark-platform-part-2.html
source_host: binary.ninja
clip_date: 2026-09-08T17:27:54+08:00
trace_id: a70d92d6-de72-4b65-aa76-e6216ddeec3b
content_hash: 20129a62414ed89e38f25309861048ce086fe8cd7a8e4626d87593ba79e3bb45
status: synced
tags:
  - Binary Ninja
  - 架构插件
...
ai_summary: |-
  为 Quark 架构实现 Binary Ninja 的 LLIL 提升器是获得高级分析与反编译能力的关键，重点是正确翻译指令并处理标志位、条件执行等语义。
  - **核心入口：** 实现 `get_instruction_low_level_il`，按 opcode 分发翻译；用 `ra_expr`/`cval` 等 helper 简化寄存器与寻址表达式，未实现指令可通过 Tags 侧栏跟踪。
  - **访存与调用：** 窄于寄存器宽度的 load 需用 `zero_extend`/`low_part` 调整；复杂寻址宜拆成临时寄存器。直接/间接 `call` 只需发 `il.call`，`lr` 写入由 Binary Ninja 自动处理。
  - **标志位设计：** Quark 无专用零/进位标志，因此为“每个标志×每种比较”定义 Flag Write Type，共 64 个再加 `addx`；需完善 `get_flag_write_low_level_il` 及 semantic flag class/group 映射，比较类多用 `sub` 提升，`z/nz` 用 `and`。
  - **特殊操作：** syscall 将指令内嵌系统调用号写入合成寄存器 `syscall_num` 后执行 `il.system_call()`；字节交换等操作定义为 intrinsics，避免分析数据流时产生无意义噪音。
  - **条件执行与边界情况：** 单条条件执行指令用 `if_expr` 配合前后 label 跳过；写入 `ip` 的指令需转成 `jump`，并在需要整函数提升时采用 `lift_function` API。
ai_summary_style: key-points
images_status:
  total: 11
  succeeded: 11
  failed_urls: []
notion_page_id: 3d575244-d011-8129-aed4-e66c072884f3
ioc: null
---
...
In [Part 1](https://binary.ninja/2026/02/20/quark-platform-part-1.html), we covered the basics of disassembly. We implemented trivial control flow recovery, patching, and even added an assembler. Thanks to Binary Ninja’s built-in ELF loader, we were able to skip writing a file format loader and spent the entire time turning instructions into tokens.

![State of the disassembly at the end of Part 1](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/624c8aa2a85c6915.png)
...
![Tags sidebar showing unimplemented instructions](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/58e939518f6cb1aa.png) *Tags sidebar showing unimplemented instructions*
...
![addx gets lifted as adc but with broken flags](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/360fb168c5f566c9.png)
...
![tags explaining that adc is looking for flags](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/649bc099f5da9be6.png)
...
![Addition with carry lifts with expressions for the carry flag](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a6e445f75373cda2.png)
...
Seeing this, you may be curious how Binary Ninja can resolve system call names and parameter types. That is explained in [Part 3’s section on calling conventions](https://binary.ninja/2026/03/04/quark-platform-part-3.html#calling-convention).
...
![The Semantic Flags system has a number of components that all relate to each other](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/88170bd934d5f46c.png)
...
![Deliberately modified function showing how reading flags directly causes the flags to lift as unimplemented](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/00770aa560e5eed7.png)
...
![Flags get resolved after implementing the flag write code](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/d6b28c89bfcce005.png)
...
![Semantic Flags diagram for a signed less-than conditional branch on aarch64](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/e49a5f7655f950e8.png)
