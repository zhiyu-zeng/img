---
title: 【微信】伪造 Object Header：一次 C++ 解释器类型混淆漏洞利用分析
source: https://mp.weixin.qq.com/s/kCNEL8eA7w9miTKTLpeclg
source_host: mp.weixin.qq.com
clip_date: 2026-09-18T18:24:41+08:00
trace_id: 1e074545-8e28-41eb-b899-4ae768bd3bf5
content_hash: 6615220852679eae9dad8c89b0f56e42e356e3d538abfb609823bb2d7e5b69a6
status: synced
tags:
  - 微信
  - CTF
...
ai_summary: 利用 stripped 64 位 C++ 解释器中的类型混淆伪造 `0xBEEF` 对象头实现 OOB 读写，逐位侧信道泄露 libc 基址后 tcache poisoning 打 `__free_hook` 拿 shell。
ai_summary_style: key-points
images_status:
  total: 11
  succeeded: 11
  failed_urls: []
notion_page_id: 3df75244-d011-81d9-a5ee-da76c39fd172
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 利用 stripped 64 位 C++ 解释器中的类型混淆伪造 `0xBEEF` 对象头实现 OOB 读写，逐位侧信道泄露 libc 基址后 tcache poisoning 打 `__free_hook` 拿 shell。
> 
> - **漏洞链核心：** BinOp 运算结果不截断可构造任意 64 位值，SetField 不校验写入值，GetField 信任 arena 中解析出的伪造 BEEF 头 count，三者叠加获得越界读写。
> - **对象模型：** 64 位 tagged value，Object 指针高 16 位 `0x1337`、低 48 位为 XOR 加密的 arena 偏移；序列化头为 `0xBEEF` 高 16 位 + 低 16 位元素数。
> - **泄露路径：** arena 初始为 0，读 `$0[0][4]` 即得 XOR key；`_IO_file_jumps` 位于 arena+0x100e8（索引 8219），解密后减 0x3e82a0 得 libc 基址。
> - **侧信道：** libc 基址右移 12 位剩 36 位，逐位乘 `0x1336<<48` 后加 `1<<48`，若高位凑成 `0x1337` 触发 "runtime error!" 即为该位 1；每位 6 条命令，共 216 条。
> - **关键坑：** 解释器不支持复合表达式（`$6=$5^$4` 结果错误）；毒化 tcache[0x30] 后不可再执行 BinOp；payload 需 24 字节前缀使 `strlen=30`、`malloc(31)` 才落 0x30 bin。

**看雪学苑** *2026年9月18日 18:01*
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/c1753e69a4f41464.svg)
...
if any(addr_bytes[i] in (0x00, 0x0a) for i in range(6)):
return False    # 坏字节，重试
...
\*本文为看雪论坛优秀文章，由 wx\_平淡无奇 原创，转载请注明来自看雪社区
