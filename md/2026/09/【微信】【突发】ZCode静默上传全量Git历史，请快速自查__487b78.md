---
title: 【微信】【突发】ZCode静默上传全量Git历史，请快速自查
source: https://mp.weixin.qq.com/s/pjojxB16hQ98JFOeTgbDGA
source_host: mp.weixin.qq.com
clip_date: 2026-09-18T16:13:18+08:00
trace_id: d11d8e09-7f3a-45f8-926a-557f0d4681c1
content_hash: 0831a9cb388f3fd39c80bfdc9d206256f448c054c3b0c421a7f9edb5c549c951
status: synced
tags:
  - 微信
  - AI应用
...
ai_summary: ZCode（智谱AI编程桌面端）在登录状态下静默把整个工作区（含完整 `.git` 历史）打包加密直传阿里云OSS，加密RSA公钥由服务端随凭证下发、私钥只在云端，UI无开关可关，只能靠系统级文件锁或字节级stub阻断。
ai_summary_style: key-points
images_status:
  total: 1
  succeeded: 1
  failed_urls: []
notion_page_id: 3df75244-d011-814d-880d-e9b12c81e0f7
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> ZCode（智谱AI编程桌面端）在登录状态下静默把整个工作区（含完整 `.git` 历史）打包加密直传阿里云OSS，加密RSA公钥由服务端随凭证下发、私钥只在云端，UI无开关可关，只能靠系统级文件锁或字节级stub阻断。
> 
> - **触发与范围：** 只要持有登录JWT，快照sidecar即无条件实例化；每次发Prompt前（`captureBeforePrompt`）与任务结束时都会抓取，单会话最多62次快照，删包半小时内自动重传。
> - **密钥姿态：** 采用信封加密（AES-256-CTR + RSA-OAEP-SHA256），公私钥均由服务端临时下发，本机密文用户与客户端自身都解不开，实质是服务端单方面可读的采集。
> - **开关无效：** "优化体验"只管是否用于训练，"仓库快照索引"只管服务端是否建索引；两者关闭后本地打包上传照旧，代码中没有针对用户配置的条件判断。
> - **数据构成：** 实测42,411文件清单中 `.git` 占86.6%（LFS缓存56.8%、objects 29.6%、reflog 0.2%），含已删除的敏感key、未推送分支名与内部GitLab地址；另有 `extra-manifest` 搭车上传全局配置，Windows侧 `model-providers.json` 存明文API key。
> - **防御手段：** macOS用 `chflags uchg`、Linux用 `chattr +i`、Windows用 `icacls` 拒绝写入锁死 `checkpoints` 目录；更彻底可对 `app.asar` 中 `captureBeforePrompt` 做等长字节替换（`if(1)return;`），依赖当前Electron fuse未开启asar完整性校验，务必备份且全程按Latin-1字节流处理。已上传数据无法召回，只能轮换密钥、重写仓库历史止损。

**星宇Sec** *2026年9月18日 15:54*
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/603958916b14c613.jpg)
...
| 开关  | 你以为它管 | 实际管 |
| --- | --- | --- |
| **优化体验**<br><br>（ `optimizeAgentExperienceEnabled` ） | 数据采集/遥测上传 | **只管要不要拿你的数据训练模型**<br><br>。关闭后快照照抓、照传 |
| **仓库快照索引**<br><br>（ `repoSnapshotIndexingEnabled` ） | 快照功能本身 | **只管服务端拿到快照后要不要建索引**<br><br>。关闭后本地打包上传一点不落 |
...
| 其余源码与文档 | ~46.2 MB | 13.4% | `src/`<br><br>、各类配置文件与业务代码 |
...
### 4.3 Windows第一层：ACL锁死投料目录（等价chattr +i）
