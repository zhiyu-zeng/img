---
title: "Lucid Dreams II: Harness Development | The Human Machine Interface"
source: https://h0mbre.github.io/Lucid_Dreams_2/
source_host: h0mbre.github.io
clip_date: 2026-09-09T00:09:04+08:00
trace_id: 2da1b513-fdd5-4871-98cc-155647ee3556
content_hash: 14d85dee0fb314ddff29e609c8626eacdc6ae42b49ec273305d25f4f3ab73170
status: synced
tags:
  - 内核Fuzzing
  - nftables
series: null
feed_source: h0mbre
ai_summary: Lucid 项目落地到 nftables 内核模糊测试：通过新增自定义系统调用、构造“信封+批处理”输入格式，并让 harness 直接调用 nfnetlink_rcv 配合 Bochs 快照恢复，实现在完全内核上下文中驱动 nftables 的深度状态模糊测试。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3d575244-d011-818f-818b-e93bafcf6ec1
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> Lucid 项目落地到 nftables 内核模糊测试：通过新增自定义系统调用、构造“信封+批处理”输入格式，并让 harness 直接调用 nfnetlink_rcv 配合 Bochs 快照恢复，实现在完全内核上下文中驱动 nftables 的深度状态模糊测试。
> 
> - **目标选择原因：** nftables 公开关注减少、代码复杂度高且有漏洞史；Syzkaller 难以生成“创建→修改→使用→销毁”之类的状态序列，作者计划自定义 mutator 提供有状态的深覆盖。
> - **新增系统调用：** 在 syscall_64.tbl 中注册 470 号 `lucid_fuzz`，在 net/netfilter/lucid_fuzz.c 中实现；用户态 harness 从 stdin 读入字节流再经该系统调用送入内核。
> - **输入格式设计：** harness 将原始字节解析为两级结构——顶层 `lf_input` 记录 `total_len`/`num_envs`，每个 `lf_envelope` 携带一条 nftables Netlink 消息；限制最多 24 个信封，每个信封数据最多 8192 字节，输入上限 196712 字节。
> - **初始化逻辑：** 通过 `netlink_table_grab` 获取注册的 NETLINK_NETFILTER 内核 socket，取出其 `netlink_rcv` 回调（nfnetlink_rcv），并预分配 24 个 `skb`，以 root 凭证填充 Netlink 控制块，供每次输入复用。
> - **Fuzz 主循环：** 以 `xchg dx, dx` 指令作为快照点，逐个解析 envelope，将数据 `memcpy` 进 `skb` 后直接调用 handler 派发给 nftables；全部解析完成后用 `xchg bx, bx` 恢复快照。用真实 `nft add table inet fuzz` 的 Netlink 消息包装后测试，harness 成功返回 0。
