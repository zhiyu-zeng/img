---
title: 【微信】AI 能自主做安全研究吗？PortSwigger 用 700 个漏洞回答：能
source: https://mp.weixin.qq.com/s/Foo5ppNW4Zh_mfE88SvOtQ
source_host: mp.weixin.qq.com
clip_date: 2026-09-22T11:34:33+08:00
trace_id: 3e2d8690-6b8c-4aa3-86c9-329011e80042
content_hash: bf5be0db9ba25b9c98cddf7df37e4c1f34bcd7039960c0130fbba5cb291ffed1
status: synced
tags:
  - 微信
  - 协议分析
...
ai_summary: PortSwigger 的 HTTP Terminator 让 AI 自主提出、验证并武器化 HTTP desync 攻击，在 3 万授权站点确认约 700 个漏洞，但最重大的发现仍需人机协作。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3e375244-d011-81e0-a930-c15405ed0b85
ioc:
  cves:
    - CVE-2026-63078
  cwes: []
...
> 💡 **AI 总结（key-points）**
>
> PortSwigger 的 HTTP Terminator 让 AI 自主提出、验证并武器化 HTTP desync 攻击，在 3 万授权站点确认约 700 个漏洞，但最重大的发现仍需人机协作。
> 
> - **四阶段流水线：** Ideation→Evaluation→Weaponization→Cascade，对应 seeker/flamer/validator/investigator 四个开源模块，输入 138 篇 RFC、1.5 万灵感片段、3 万去重攻击向量。
> - **微灵感反锚定：** RFC 被拆成 1～3 句片段，提示词刻意回避 desync/smuggling 等关键词；上下文越多模型锚定越死，protocol-ruler 再发现成功率一度为 0%，换新模型后升至 30%。
> - **评估是核心：** Burp 扩展 + SQLite，2000 线程 7×24 攻打 3 万授权站点、限速每域每秒 1 请求；只发正常请求加另一连接的触发器，观察响应是否被污染，对"污染响应长什么样"零预设。
> - **战果：** 约 700 个漏洞系统，含美国政府站点与多家银行；multipart/byteranges 触发器命中 200+ 站点；Apache Traffic Server 零日 CVE-2026-63078；AI 自主产出新武器化技术 dangling-byte，并发现"响应分叉"新类别证据。
> - **人机分工与黑魔法：** 攻击成败判定交由确定性代码实现零误报；绕过模型护栏靠改名 Turbo Simulator、假开关、隐藏 Connection: close、改用自创术语；最重大发现 Shared-Parser Confusion（响应处理特性可被请求利用）由 AI 提假设、Kettle 验证。

**网络安全透视镜** *2026年9月21日 23:53*
