---
title: Patch-Gapping the Google Container-Optimized OS for $0 | The Human Machine Interface
source: https://h0mbre.github.io/Patch_Gapping_Google_COS/
source_host: h0mbre.github.io
clip_date: 2026-09-09T00:06:40+08:00
trace_id: 133e0a58-0747-4940-ae3c-d7bea7f44782
content_hash: 9572949043d2902f0020cf194da99ec2fb4d5b2d0a686a8fc5760bd09c32726f
status: synced
tags:
  - 内核
  - 漏洞分析
series: null
feed_source: h0mbre
ai_summary: TL;DR：作者针对 kCTF COS 105 上未修补的 net/sched qdisc 重挂载 UAF 漏洞，构造出通过劫持 drr_dequeue 间接调用并覆盖 modprobe_path 完成提权的 1day exploit。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3d575244-d011-815a-87af-d54dfe757934
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> TL;DR：作者针对 kCTF COS 105 上未修补的 net/sched qdisc 重挂载 UAF 漏洞，构造出通过劫持 drr_dequeue 间接调用并覆盖 modprobe_path 完成提权的 1day exploit。
> 
> - **漏洞背景：** 该问题由 Lion Ackermann 发现，对应内核补丁为 bc50835e83f60f56e9bec2b392fb5544f250fb6f；核心是 `tc qdisc replace dev lo parent 1:3 handle 4:0` 把一个已挂在 class 3:1 上的 qdisc 再挂到 class 1:3，导致同一 qdisc 被两个 class 引用、refcount 变为 2。
> - **调试弯路：** 作者本地内核开启了 `CONFIG_BUG_ON_DATA_CORRUPTION`，无效 `list_del` 会直接 BUG/panic；而 kCTF 内核未开启该选项，只会 WARN，因此他数日卡在 `list_del corruption` 上，误以为 PoC 无法走到真正的 UAF。
> - **UAF 根因：** 真正被 UAF 释放的是 `drr_class` 对象而非 qdisc；删除 class 1:1 时，`drr_qlen_notify` 会对 active list 中已释放 class 的 `alist` 执行 `list_del`，访问到已被释放的 class 1:3 内存。
> - **利用原语：** 触发窗口位于 `drr_dequeue` 的 `cl->qdisc->ops->peek()` 间接调用，可劫持执行流；作者因找不到合适 stack pivot 放弃 ROP，改为组合内核固定可写区域 `kernfs_pr_cont_buf` 与自选 gadget，形成任意增量写原语。
> - **最终提权链：** exploit 用 Entrybleed 解 KASLR，用 `nft_table->udata` 配合 kmalloc-128 cross-cache 堆喷重占释放的 class，再通过管道页回填 fake class/qdisc，反复逐字节将 `/sbin/modprobe` 改为容器内可预测路径 `/proc/500/fd/3`，最终触发 modprobe 执行读取 /flag。
