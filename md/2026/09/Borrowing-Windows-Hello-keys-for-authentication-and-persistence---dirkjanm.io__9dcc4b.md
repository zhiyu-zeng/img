---
title: Borrowing Windows Hello keys for authentication and persistence - dirkjanm.io
source: https://dirkjanm.io/borrowing-windows-hello-keys/
source_host: dirkjanm.io
clip_date: 2026-09-25T10:19:24+08:00
trace_id: a9423099-6882-4b50-a317-cfb323b09f8a
content_hash: 4cc01127d5637b047d5431a6bfec0f3880182da72f8ecaf1cae0a9fce9130232
status: synced
tags:
  - 协议分析
  - 风控对抗
...
ai_summary: 攻击者可从被控会话调用 Windows Hello for Business 密钥，无需 PIN/生物识别即可伪造 WebAuthn/FIDO 断言，进而获取 PRT、注册设备并持久化。
ai_summary_style: key-points
images_status:
  total: 8
  succeeded: 8
  failed_urls: []
notion_page_id: 3e675244-d011-81d4-a947-c8c593d1fd06
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 攻击者可从被控会话调用 Windows Hello for Business 密钥，无需 PIN/生物识别即可伪造 WebAuthn/FIDO 断言，进而获取 PRT、注册设备并持久化。
> 
> - **前提条件：** 需访问受害者会话（如植入物），另需一个能注册/加入 Entra ID 的账号；PRT 流程需伪造设备，FIDO 流程可绕过该要求。
> - **技术要点：** 低权限可用 PowerShell 调 `Ncrypt.dll` 与 Passport KSP，经 `NCryptOpenKey` 加载 WHFB 证书密钥并签名本地 JWT；断言仅 5 分钟，换取的 PRT 90 天且可续期。
> - **WebAuthn 关键：** `user_handle` 由 `ON:` + 租户 ID（小端二进制）+ 用户 ID 的 SHA256（同格式）拼接；挑战为 Entra ID 签名 JWT，前缀 `O.`，5 分钟且不绑定会话/用户/租户。
> - **持久化链：** 无设备 ID 声明的 FIDO 令牌可注册新设备，再请求 broker refresh token 注册设备并请求 PRT；使用 WHFB 算作新 MFA，可加后门 passkey。
> - **检测：** KQL 查 `SigninLogs` 中认证方法为 WHFB 且 `DeviceDetail.deviceId` 为空；更精准版本过滤跨租户类型为 none、认证详情长度为 1、无 AuthContext。工具在 ROADtools 的 `winhello_assertion`，`roadtx fidoauth`/`registerpasskey` 已支持。
...
![Windows Hello for Business key linked to its device](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/f94f702d2198a60c.png)
...
![Gathering the signed assertion](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/fa8a3ecccbb00465.png)
...
![Requesting a PRT with the assertion (assertions shortened for readability)](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/c68d9c5a3d3659ca.png)
...
![FIDO challenge](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/64d76b88b4ba1b9b.png)
...
![FIDO authentication PowerShell script](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/be22abf67eaddca2.png)
...
![FIDO assertion to token](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/4bc5d64685062b2a.png)
...
![Device claim absent in FIDO token](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/3c92e955aad540fd.png)
...
![FIDO assertion to device to PRT](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a7cc2c30e38f0fe8.png)
