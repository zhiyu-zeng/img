---
title: "Binary Ninja - Building a Custom Architecture and Platform: Part 3"
source: https://binary.ninja/2026/03/04/quark-platform-part-3.html
source_host: binary.ninja
clip_date: 2026-09-08T17:29:12+08:00
trace_id: 19ddddf1-1a07-477b-b3d4-331b4c3ee9bb
content_hash: b7c58693aeb0c68030461e74205dc221dd5a5f579f9da1eeda1fe775e417c786
status: synced
tags:
  - 二进制分析
  - WARP
series: Binary Ninja - Building a Custom Architecture and Platform
feed_source: Binary Ninja Blog
ai_summary: Quark 架构插件通过实现 Platform、调用约定、类型库和 WARP 签名，可将反编译输出提升至接近源码级别，并自动标注系统调用与静态链接库函数。
ai_summary_style: key-points
images_status:
  total: 19
  succeeded: 19
  failed_urls: []
notion_page_id: 3d575244-d011-819f-a80d-e1faea46410c
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> Quark 架构插件通过实现 Platform、调用约定、类型库和 WARP 签名，可将反编译输出提升至接近源码级别，并自动标注系统调用与静态链接库函数。
> 
> - **调用约定：** 通过声明 caller_saved_regs、int_arg_regs、int_return_reg 等字段即可让函数调用参数被正确识别；示例中仅列出 8 个整型参数寄存器 `r1-r8`，并将 `arg_regs_for_varargs=False` 指明可变参数走栈。
> - **系统调用：** 需单独的 syscall 调用约定，把 `syscall_num` 作为首个 int_arg_reg，且设置 `eligible_for_heuristics=False` 防止无类型调用被误判为系统调用。
> - **平台类型与类型库分工：** Platform Types 只放 `noreturn` 系统调用和函数（如 `sys_exit(1)`、`sys_exit_group(252)`）；其余海量类型放入 Type Library，按需加载，常用做法是把系统调用库命名为 `SYSCALLS`，插件可用 Python API 从 C 头文件生成 `.bntl` 并注册。
> - **静态链接函数匹配：** 用 WARP 由标注过的分析生成函数签名；Quark 的编译器 SCC 易内联函数，可通过在 harness 函数末尾添加大量 `puts("")` 防止内联。
> - **IP 相对寻址导致的签名不匹配：** 相对常量由多条指令合成会使 WARP 无法识别为 variant；Quark 通过克隆 `core.function.metaAnalysis` 工作流，在 generateLiftedIL 后插入活动，将 `rA=const; rB=ip+const; rA=rB` 重写为 `rA=const+ip; rB=const+ip` 并加 nop，成功让 `puts()` 在被测二进制中匹配签名。

![Quark](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/ecfe7c2878ff241c.png)
...
The complete source code for this series is available at [github.com/Vector35/arch_quark](https://github.com/Vector35/arch_quark).
...
![Function calls have proper arguments now](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/d06891509a2197ba.png)
...
![Now we can see it's doing syscall number 4](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/41ecad4dacc84a4e.png)
...
![sys\_exit now gets its name and type](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/05457678f787d56c.png)
...
![Windows's ntdll.dll Type Library will be loaded if an executable links against api-ms-win-core-crt-l1-1-0.dll, among other names](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/fbcf4dfb0ee3351a.png)
...
![System calls now have their names and types annotated](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/98307cb455b47a90.png)
...
![The standard library's Type Library is available, and we can use its types](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/504a04fe42ad0bad.png)
...
![Windows's mfc42.dll references all of its imported functions by ordinals, which can be seen in the Type Library Explorer](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/8c60cb32e71e9b23.png)
...
![Analyzed standard library calls with names and types](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/78bf2eb0676ee4b1.png)
...
![Statically linked standard library functions now have their names and types annotated for us](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/8dfd9a0238c0a765.png)
...
![0x52ad is an ip-relative offset, and we need WARP to exclude it from signature generation](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/e454977d128a9fe0.png)
...
![This version of puts() uses a different constant and doesn't get matched.](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/b9bc08313a195fb1.png)
...
![The ip-relative constant load is marked as variant now](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/bed8ebbbf21b9160.png)
...
![The other instruction is also variant, even if the Render Layer doesn't notice](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/6a3df1b31c83aa4f.png)
...
![puts gets matched now](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a29d6b9af38f81be.png)
...
![One-shot analysis can make simple binaries look like source code with no user input](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/5f721a0b8b22a5c8.png)

One-shot analysis can make simple binaries look like source code with no user input

![User annotations allow for complex structure access recovery and analysis](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/ab546b57676c8eb8.png)

User annotations allow for complex structure access recovery and analysis

![Control flow recovery and calling convention support show how a payload parses its own memory map](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/51788d362c5370d1.png)

Control flow recovery and calling convention support show how a payload parses its own memory map

## Conclusion

We hope this series helps you write custom architectures of your own and use some of the more advanced parts of Binary Ninja to improve your analysis. Enabling you to get professional-grade decompilation for obscure architectures has been one of our primary goals, since we can’t possibly have official support for every architecture. It may not be trivial, but there are a bunch of resources online to help you along the way. If you want to read more, you can check out [our previous blog series](https://binary.ninja/2020/01/08/guide-to-architecture-plugins-part1.html) [about Z80](https://binary.ninja/2021/12/09/guide-to-architecture-plugins-part2.html), our [open-source official plugins](https://github.com/Vector35/binaryninja-api/tree/dev/arch) [and examples](https://github.com/Vector35/binaryninja-api/blob/dev/python/examples/nes.py), and [some](https://github.com/samrussell/binja-gameboy) [of](https://github.com/galenbwill/binaryninja-m68k) [our](https://github.com/Accenture/NEC850_Architecture) [community](https://github.com/nicabi/binja-xtensa2) [plugins](https://github.com/otter-sec/bn-ebpf-solana). The source code for this series is available [on our GitHub](https://github.com/Vector35/arch_quark) as well.
