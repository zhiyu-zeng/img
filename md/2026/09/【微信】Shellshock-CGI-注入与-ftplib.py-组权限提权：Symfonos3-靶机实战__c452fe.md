---
title: 【微信】Shellshock CGI 注入与 ftplib.py 组权限提权：Symfonos3 靶机实战
source: https://mp.weixin.qq.com/s/7qdfoonhuuUc9bcWGuHqxA
source_host: mp.weixin.qq.com
clip_date: 2026-09-22T11:33:32+08:00
trace_id: 88fb7574-c113-4279-b31b-2b74cb20ab13
content_hash: ba46031e95c29f2cff4c27be04392fce80d22cda1e5e741fa7539a7e8184b309
status: synced
tags:
  - 微信
  - CTF
...
ai_summary: Symfonos3 靶机演示了一条完整攻击链：借助 CGI 的 Shellshock 注入获取 www-data shell，再靠 pspy 与 tcpdump 挖出 root 定时任务与明文凭据，最终篡改 Python 标准库提权至 root。
ai_summary_style: key-points
images_status:
  total: 7
  succeeded: 7
  failed_urls: []
notion_page_id: 3e375244-d011-81b6-b9b5-d4be8eae2065
ioc:
  cves:
    - CVE-2014-6271
  cwes: []
...
> 💡 **AI 总结（key-points）**
>
> Symfonos3 靶机演示了一条完整攻击链：借助 CGI 的 Shellshock 注入获取 www-data shell，再靠 pspy 与 tcpdump 挖出 root 定时任务与明文凭据，最终篡改 Python 标准库提权至 root。
> 
> - **整体攻击链：** 目录枚举发现 CGI → Shellshock 环境变量注入拿 foothold → pspy 监控 root cron → tcpdump 嗅探回环 FTP 明文凭据 → SSH 横向移动 → 掺改 Python 库文件提权。
> - **入口定位：** 主页源码注释 "Can you bust the underworld?" 暗示隐藏路径；dirb 显示 /cgi-bin/ 返回 403 而非 404，gobuster 递归扫出 /cgi-bin/underworld，访问后直接回显 uptime 结果，证明服务端在跑系统命令。
> - **Shellshock 原理与利用：** CVE-2014-6271，Bash 会把 `foo=() {:; }; 命令` 这类环境变量当函数定义解析并继续执行尾随命令；Apache 又将 User-Agent 等请求头自动映射为环境变量、CGI 由 Bash 解释，三条件齐备，注入后反弹 www-data shell。修复方式为升级 Bash 至 4.3+ 或对 CGI 环境变量做白名单过滤。
> - **提权侦察：** 无 root 权限时 `ps aux` 信息不全，pspy64 靠读 /proc 捕获到 UID=0 执行的 `/usr/bin/python2.7 /opt/ftpclient/ftpclient.py` 定时任务；`tcpdump -i lo` 抓到明文 `USER hades` / `PASS PTpZTfU4vxgzvRBE`，凭据复用于 SSH 成功登录。
> - **库文件劫持提权：** /usr/lib/python2.7/ftplib.py 权限为 root:gods 且组可写，hades 恰属 gods 组，在文件头部插入 `os.system` 反弹 shell，root 的 cron 下次加载该模块即触发，获得 uid=0。防御上标准库应设为 root:root 644，且避免在定时脚本中硬编码明文密码。

**Hash-Sec** *2026年9月21日 19:15*
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/f471d1510141b484.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/06027d1172fecd56.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a2a1f018748f98d7.png)

Kali 监听端口收到来自目标的反弹连接，获得 www-data 权限的交互式 shell。

![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/b60e1c11252dd873.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a1b414a259350bd6.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/083efe94d11f1cd9.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/057b5212b8d6197d.png)
