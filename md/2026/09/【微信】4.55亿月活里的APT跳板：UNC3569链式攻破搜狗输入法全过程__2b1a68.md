---
title: 【微信】4.55亿月活里的APT跳板：UNC3569链式攻破搜狗输入法全过程
source: https://mp.weixin.qq.com/s/6jED7tURasxPRpl4Kr6wLg
source_host: mp.weixin.qq.com
clip_date: 2026-09-12T09:06:03+08:00
trace_id: c6c13cae-2dc1-4910-8933-4404853d58d7
content_hash: 5743cde43e2a5b199460e92f9568dab5e686e381a1ddeccfae18b26779335338
status: synced
tags:
  - 微信
  - 漏洞分析
...
ai_summary: UNC3569 借搜狗输入法一键 shell 链（CVE-2026-51990）在 4.55 亿月活用户上投递 GRAYRABBIT 后门，腾讯只堵了入口、内嵌浏览器仍是 2020 年 Chromium 80。
ai_summary_style: key-points
images_status:
  total: 3
  succeeded: 3
  failed_urls: []
notion_page_id: 3d975244-d011-81da-bbd8-f4a541df12e4
ioc:
  cves:
    - CVE-2021-38003
    - CVE-2026-51990
...
> 💡 **AI 总结（key-points）**
>
> UNC3569 借搜狗输入法一键 shell 链（CVE-2026-51990）在 4.55 亿月活用户上投递 GRAYRABBIT 后门，腾讯只堵了入口、内嵌浏览器仍是 2020 年 Chromium 80。
> 
> - **三步链式利用：** 自定义协议 `sgbiz:` 传递参数时 `biz_helper.exe` 不校验命令行参数启动任意组件；`SGMyInput.exe` 的皮肤商店不校验 URL 可跳转任意站点；内嵌 Chromium 80 在代码中关闭沙箱与同源策略，使 CVE-2021-38003（V8 `JSON.stringify` 内存破坏）EXP 直接生效，构成"点击即 shell"。
> - **载荷投递：** 从阿里云香港 8.218.50.207 下载合法 7-Zip、恶意 `7z.dll`（DLL 搜索路径劫持）与加密载荷 `p` 至 `C:\Users\Public\Documents\`；压缩命令本身无意义，仅为触发 DLL 加载。
> - **反分析与隐蔽：** 恶意 DLL 解密前统计进程数，少于 50 个即使用错误密钥使 payload 失效；DLL 借 NTFS 备用数据流自删除；C2 为 mail.uaiubifas.top:443，端口 443 上跑的是 RC4 明文 TCP 而非 TLS。
> - **修复仅一半：** 16.3.0.3498 只在 `biz_helper.exe` 增加参数校验（仅 HTTPS、限 4 个域名后缀），沙箱与同源策略开关依旧关闭、Chromium 仍为 80 版；CISA KEV 中 41 个 V8 漏洞至少 32 个的修复版本晚于该内置版本。
> - **防御要点：** 升级至 16.3.0.3498+；告警 443 端口非 TLS 流量；监控 Public 目录可疑 DLL、合法 7-Zip 与该目录的异常出现及 NTFS 备用数据流；DNS 阻断 uaiubifas.top、noht1ng.top。

**黑白之道** *2026年9月12日 08:30*
...
![搜狗输入法Windows版产品示意](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/b09d47fd08adaae8.jpg "搜狗输入法Windows版产品示意")
...
5.  **DNS层阻断**：阻断 uaiubifas\[.\]top、noht1ng\[.\]top
    
6.  **建议腾讯**：尽快把内嵌Chromium升级到当前版本，并开启沙箱——这是治本之策
