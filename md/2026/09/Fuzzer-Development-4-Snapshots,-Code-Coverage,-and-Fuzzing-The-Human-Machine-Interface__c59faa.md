---
title: "Fuzzer Development 4: Snapshots, Code-Coverage, and Fuzzing | The Human Machine Interface"
source: https://h0mbre.github.io/Lucid_Snapshots_Coverage/
source_host: h0mbre.github.io
clip_date: 2026-09-09T00:05:57+08:00
trace_id: 0680838e-374b-42fd-85db-99bad42636b6
content_hash: 1f75be6b54ba55d2779d0a7fe605d4d45fc67ab74768ddadfae4c913b2e54fcd
status: synced
tags:
  - 模拟执行
  - 模糊测试
series: null
feed_source: h0mbre
ai_summary: Lucid 快照模糊器已推进到可实际 fuzz 的阶段：用 Bochs 封装目标，借助连续内存块和 shm+mmap 实现低成本快照恢复，并加入基于分支边沿的覆盖反馈。
ai_summary_style: key-points
images_status:
  total: 2
  succeeded: 2
  failed_urls: []
notion_page_id: 3d575244-d011-8134-869b-d38fd62ef0df
ioc:
  cves: []
  cwes: []
  hashes:
    - "13371337133713371338133813381338"
  domains: []
  tools: []
  techniques: []
---
...
> 💡 **AI 总结（key-points）**
>
> Lucid 快照模糊器已推进到可实际 fuzz 的阶段：用 Bochs 封装目标，借助连续内存块和 shm+mmap 实现低成本快照恢复，并加入基于分支边沿的覆盖反馈。
> 
> - **快照恢复方案：** 将可写 ELF 段、扩展 CPU 状态和预分配 MMU/Brk 池整理成一段连续内存，用 `shm_open` 共享内存对象缓存快照，恢复时直接 `MAP_FIXED`+`mmap` 覆盖脏内存；在测试环境中约 1.8 万次/秒，比整块 `memcpy` 快约 10 倍，避免每次迭代对 2GB 级内存做大复制。
> - **触发快照的 NOP 指令：** 复用 Bochs 的调试魔数指令，如 `xchg dx, dx` 让带 GUI 的 Bochs 保存状态到磁盘；fuzzer 内用 `xchg bx, bx` 表示结束本次输入并回到快照状态，崩溃则在 harness 中用特殊 NOP+除零指示。
> - **覆盖反馈实现：** 编译 Bochs 时开启 `BX_INSTRUMENTATION`，修改分支 stub 让 not_taken 也携带目标 PC，将 `(branch_eip, new_eip)` 用 `dbj2_hash` 映射到 64KB `u8` 数组，统计边沿命中次数；Lucid 端按 AFL++ 的分桶策略对比历史最高桶值，判断是否发现新覆盖。
> - **目标环境搭建：** 自定义 Linux 6.0.1 内核并加入 syscall 编号 451 的 `fuzzme`，配合 BusyBox initramfs 打成 ISO；先在 GUI Bochs 中启动执行 harness，到快照点后存盘给 Lucid 恢复使用，并通过内存中的 128-bit signature 定位可变输入数据。
> - **待办项：** mutator 目前只是演示占位，计划接入 Brandon Falk 的 basic_mutator；corpus 管理、并行化/分布式、Redqueen 支持以及未来作为 LibAFL 执行引擎仍是后续工作。
...
![Bochs Boot](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/4c94c05dc48267ef.png)
...
![Lucid Stats](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/859d18b1b04ac639.png)
