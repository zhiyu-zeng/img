---
title: 【微信】OLLVM学姐攻略手册
source: https://mp.weixin.qq.com/s/gCfuyyw5wx5meUoAfynBTg
source_host: mp.weixin.qq.com
clip_date: 2026-09-19T23:15:02+08:00
trace_id: a6e7dea9-f0a7-4009-9549-bce6977f7bec
content_hash: f273dc17f3ccbded9b88041f24c1bf78fd717de28cd9f64bb6d07949d24e57dc
status: synced
tags:
  - 微信
  - Android逆向
...
ai_summary: OLLVM 反混淆的核心是找到那个"决定走向的值"——MBA 表达式、不透明谓词、状态变量——判断真值后把控制流接回；静态匹配图快、动态执行图稳，混淆越强越要退回动态。
ai_summary_style: key-points
images_status:
  total: 63
  succeeded: 62
  failed_urls:
    - https://mmbiz.qpic.cn/sz_mmbiz_png/Cpo2XCpI7K2Ebp1wwEQkOXxkLnl4fDqazGTicpqiaOt9wIZr1XSARtH0OiabJEgj78EFOctXic9MejcRl878AAoXGhJ2UJA7e9VJgL76YMYERTI/640?wx_fmt=png&from=appmsg#imgIndex=20
notion_page_id: 3e075244-d011-8130-9f78-ff73b4dba4c0
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> OLLVM 反混淆的核心是找到那个"决定走向的值"——MBA 表达式、不透明谓词、状态变量——判断真值后把控制流接回；静态匹配图快、动态执行图稳，混淆越强越要退回动态。
> 
> - **版本差异：** OLLVM 非固定工具，差异在底座 LLVM 版本（4~17）、Pass Manager（Legacy/New）、开关名（-fla vs -irobf-cff）、Pass 种类（可含字符串加密、间接跳转）。本文选 LLVM 16.0.6 + wwh1004 的 ollvm-16。
> - **样本编译：** 编译带混淆 clang（`-DLLVM_OBFUSCATION_LINK_INTO_TOOLS=ON`），用 NDK r27d 以 `--target=aarch64-linux-android21` 生成 rc4 的 base/sub/bcf/fla 四个对照 .so。
> - **指令替换还原：** D-810 规则匹配后残留的 MBA 表达式交 GAMBA，输入必须是单右值表达式（去赋值号、剥数组下标与指针解引用、统一内存变量名、显式补括号），命令格式 `simplify_general.py "expr" -b 位宽 -z 1`；位宽取能容纳最大常量的最小宽度。
> - **BCF 四条路径：** D-810 的 bogus_loops 最省事；常量传播剪枝（改 .bss 为只读并 patch 为定值，依赖 IDA 死代码消除）要求谓词变量专用且同段无真实全局变量；deflat 符号执行因循环次数依赖符号值路径爆炸，实测跑不动；qiling 污点追踪最通用——监控 x/y 全局只读变量，追踪 NZCV/寄存器传播，判定条件跳转脏净后把假跳转 patch 成 B 指令（注意 RVA 转文件偏移）。
> - **FLA 三条路径：** D-810 需修复原项目"右操作数非立即数不推进迭代"的死循环 bug；状态机法静态建表 A（块→下一状态）与表 B（状态→块），把写状态+跳分发器改成直连，前提是状态值为简单 MOV 立即数；angr 不猜状态值，逐真实块分段跑并 hook 序言末指令跳过分发器，但要手动分裂 CSEL 分支并防路径爆炸。

**看雪学苑** *2026年9月19日 18:04*
...
本文不研究这些 Pass 怎么实现，本文以1万6千字详细记录了面对不同 Pass 时如何落地一套还原思路，据我所知对抗办法并非只有本文列出的内容，还有Triton、unidbg等工具，碍于篇幅没有将所有的工具都做实践记录。
...
void rc4_crypt(const unsignedchar *key, size_t key_len,
unsignedchar *data, size_t data_len) {
unsignedchar S[256];
int i, j;
// KSA
for (i = 0; i < 256; i++) S[i] = (unsignedchar)i;
    j = 0;
for (i = 0; i < 256; i++) {
        j = (j + S[i] + key[i % key_len]) & 0xFF;
unsignedchar tmp = S[i]; S[i] = S[j]; S[j] = tmp;
    }
// PRGA
    i = j = 0;
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/fefa935526c26ecc.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a4d7258f494a780c.png)

重启ida在插件内就可以找到D810了 打开后长这样

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/39ce3be0f12d226a.png)

关于配置的介绍可以去看目录下的介绍

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/5a43df77e44d4093.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/59adf3ebea7e2ebf.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/d8201e65e6aaf5f5.png)

经 `-sub` 编译后，反编译结果的运算被替换为等价但晦涩的形式

### D-810 批量还原

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/30972b30d87ed543.png)
...
运算域：
若表达式中某变量做的是完整宽度运算（存在超过 8 位的常量，或明显的宽位运算），只是最后才取低字节，则按该变量在"变量类型"清单里的声明类型定位宽（int/unsignedint/_DWORD→32，__int64/_QWORD/指针→64）。不要因为末尾取字节就降低位宽。
...
类型映射参考：char/_BYTE/__int8→8，short/__int16→16，int/unsignedint/_DWORD→32，__int64/_QWORD/指针→64。
...
unsignedint v10; // [xsp+28h] [xbp-138h]
unsignedint v11; // [xsp+28h] [xbp-138h]
int i; // [xsp+2Ch] [xbp-134h]
int j; // [xsp+2Ch] [xbp-134h]
unsignedint v14; // [xsp+2Ch] [xbp-134h]
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9eb555b7aabc2016.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/4483834a3cacc256.png)
...
下面是混淆前后的对比，可以明显的看到相比原版的流程图新增了多条分支

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/756583eda5d5f59a.png)

那么面对bcf又该怎么处理呢？根据目前网上大佬们公开的方案有以下几种

### 思路一:D-810 插件

这个最简单粗暴 选择 **`bogus_loops.json`** 后F5刷新

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/909ffe871303cad6.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/2142696a8f6a23ab.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/1d1dc0972be1cdc5.png)

将权限设置为只读

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/57509bf71fd7b4b6.png)

\*\*第四步 patch

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/3fccc2e349c716e9.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/6b052e85450d7f61.png)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a1cbb9186dab2b92.png)

回到函数F5刷新一下就自动消除了 核心是依赖于IDA 的死代码消除 (DCE, Dead Code Elimination)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/c1b3af8c22661f48.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/93a0f2088c4d2a01.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/f2a7ed8319f0120d.png)
...
![⚠️ 图片托管失败 · image](https://mmbiz.qpic.cn/sz_mmbiz_png/Cpo2XCpI7K2Ebp1wwEQkOXxkLnl4fDqazGTicpqiaOt9wIZr1XSARtH0OiabJEgj78EFOctXic9MejcRl878AAoXGhJ2UJA7e9VJgL76YMYERTI/640?wx_fmt=png&from=appmsg#imgIndex=20)

在bogus_control_flow目录下中运行命令

```
python debogus.py -f librc4_bcf.so--addrx401648
```
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/21d035df4b03a30e.png)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/e6399cc5c3003fb5.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/14fd3fc720d3501b.png)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/3b1dbb9d4475aa3f.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/d1c88886721e8b65.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/3b6b03a491c3bc9a.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/7496b5ad94467515.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9425d1999bfcb417.png)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/eec2c10a289e356e.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/5319ec8e4b8598c4.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9c39dcae12364085.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/bd31d30334c8c02d.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/92b9c6a0af48d671.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9f1623e1d2996eb6.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/2c26936e4e7769f8.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/58c5b14b4b9b31fa.png)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/7503de282e178268.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/6076320a81961eb5.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/be526b677c787af8.png)

### 思路一：D810

依旧“脚本小子”打法 没办法 太爽了 直接一把梭干净了

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/5425bd5d464a6a51.png)

选中点击flatfold.json start后切换到伪代码窗口 会发现ida卡死 切换到default_unflattening_ollvm.json发现也是一样的

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/90d571d06e67d8aa.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/ed131491bff27057.png)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/e98d0feabdfa3bb4.png)
...
分发器 分发器的特征就是每个块执行完毕都会跳回到这里 它被跳转的次数最多
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/556e93728f76a010.png)

状态值 这个去看真实块的屁股 在前面提到了 状态值这玩意 每个块执行完毕了 就会更新然后跳转到分发器 那么不难看出这里跳转之前我们的var_150同学也是备受瞩目好吧 除此之外简单分析一波loc_171c的汇编代码。
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9f0c2d04c758cc0e.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/c511d3fe4d4076d1.png)

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a21f99a58e8d0f83.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/b30fbbe8e6b95e7a.png)

交叉引用看看怎么个事

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a2b275d9a822e07f.png)

真相大白了 这一串的STR与MOV解释了其实还是一样的讨论 只不过套个娃给你看罢了

![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/5fb4ba8fb4cfa63e.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/ad5c6f2dda1107dd.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/6c2ee82cdb7706db.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/e1934b5fa92d2b87.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/1a176bf75b07711a.png)
...
![image](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/bbd8c9b6b8bb5f47.png)
...
OLLVM 反混淆到底是同一个动作：找到那个"决定走向的值"，判断真假，再把控制流接回去。指令替换里是 MBA 表达式，虚假控制流里是不透明谓词，平坦化里是状态变量。工具只是手段：静态匹配赌它形态固定，图快；动态执行不赌，跑一遍看实际取值，图稳。样本越简单越该用静态，混淆越强越得退回动态。本文的方法并非原创，而是站在前人的肩膀上。特别感谢参考文献（左下角阅读原文查看）中各位前辈的公开记录，希望本文能够帮助到大家。

![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/755a8a52bffa2682.png)

看雪ID：北袅

https://bbs.kanxue.com/user-home-951483.htm

\*本文为看雪论坛优秀文章，由 北袅 原创，转载请注明来自看雪社区
