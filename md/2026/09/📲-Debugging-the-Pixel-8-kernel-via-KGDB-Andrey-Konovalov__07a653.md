---
title: 📲 Debugging the Pixel 8 kernel via KGDB | Andrey Konovalov
source: https://xairy.io/articles/pixel-kgdb
source_host: xairy.io
clip_date: 2026-09-09T10:17:08+08:00
trace_id: 94a8ec30-48c6-4e68-bbda-2d59a70f5bd5
content_hash: e45345613d027e17c2fe93a2fd94b7f4ffb039d9c8afa86e2a1320eb5303ac55
status: synced
tags:
  - Android逆向
  - 内核
series: null
feed_source: Andrey Konovalov·xairy
ai_summary: Pixel 8 可用串口 KGDB+GDB 调试内核；需开放 UART、编译并刷入开启 KGDB 的内核，经 ADB 或串口触发 KGDB，同时处理看门狗与 PAC 等干扰，Android 15/16 下均验证可行。
ai_summary_style: key-points
images_status:
  total: 4
  succeeded: 4
  failed_urls: []
notion_page_id: 3d675244-d011-81b6-ac2e-d9da13bff5d1
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> Pixel 8 可用串口 KGDB+GDB 调试内核；需开放 UART、编译并刷入开启 KGDB 的内核，经 ADB 或串口触发 KGDB，同时处理看门狗与 PAC 等干扰，Android 15/16 下均验证可行。
> 
> - **硬件前提：** Pixel 的 UART 串口在 Type-C SBU1/2 引脚（1.8V），需 USB-Cereal 转接；Google 原版采用 FT232R\* 芯片，0xDA 版本多用 CP2102N，无法发送进入 KGDB 所需的 break 序列。
> - **编译与刷入：** Pixel 8（shusky）内核源码用 repo 拉取；需改 kleaf/kgdb.bzl 后执行 `./build_shusky.sh --config=pixel_debug_common --kgdb --lto=none`，再用 fastboot 关闭 verification 并刷 boot/dtbo/vendor_kernel_boot/vendor_dlkm/system_dlkm。
> - **参数与看门狗：** 用 `fastboot oem cmdline set` 设置 `kgdboc=ttySAC0,115200 nokaslr sysrq_always_enabled ehld.noehld=1 s3c2410_wdt.soft_noboot=1 arm64.nopauth`；若不关 EHLD/APC watchdog，进入 KGDB 约 20 秒即被重启。
> - **两种触发方式：** 经 ADB 写 `/proc/sysrq-trigger` 需先修补内核绕过 SELinux/DAC；经串口发 SysRq-G 则须先 `cat /dev/ttySAC0` 激活驱动，且 `/proc/sys/kernel/sysrq` 为 1。串口方式不占用 USB-Cereal 直通口，便于连接恶意 USB 设备调试。
> - **GDB 实测要点：** 普通断点可用，条件断点偶发导致 GDB 崩溃；单步需自定义 `my-si` 宏避免进入中断；内核模块用 lx-lsmod + `add-symbol-file` 加载；扩展 KGDB 暴露系统寄存器后，GEF 的 slub-dump/buddy-dump 等命令在 KGDB 模式也可用。
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/6779178429cc0471.jpg "Left to right: 1. USB-Cereal fabricated based on original Google design;<br>2. 0xDA's version of USB-Cereal; 3. Private version of USB-Cereal")

Left to right: 1. USB-Cereal fabricated based on original Google design; 2. 0xDA's version of USB-Cereal; 3. Private version of USB-Cereal
...
Go to `Settings` ➡️ `About phone` and click 7 times on `Build number`. This unlocks the `Developer options` menu.

Go to `Settings` ➡️ `System` ➡️ `Developer options` and enable `USB debugging`. This enables ADB to be used with the phone.
...
Go to `Settings` ➡️ `System` ➡️ `Developer options` and enable `OEM unlocking`.
...
Even with a proper USB-Cereal, when I first tried sending the SysRq-G sequence via `minicom` (`Ctrl+a`, then `f`, then `g`), nothing happened 😢
...
Once I ran these commands, suddenly sending SysRq-G via `minicom` partially worked 😮:
...
With this approach, the previous `aosp/` patches that disabled SELinux and unrestricted `chmod` could be reverted.

But note that `filp_open` in the patch above would be restricted by SELinux. I’ll leave it up to the reader to devise a patch that works around that.
...
I also tested the favorite GDB extension of Linux kernel hackers — the [bata24’s GEF work](https://github.com/bata24/gef).

**Fail**. With this extension enabled, I could connect to the kernel over KGDB, but there was a crash coming from the extension’s internals.

Click the switch to see the crash.

And even though this crash did not prevent the use of the GEF commands, unfortunately, the `slub-dump` command was disabled in the KGDB mode:
...
**Root causes**. After poking around the GEF source code, I found two the main reasons GEF disabled `slub-dump` (and many other commands) in the KGDB mode:

1.  KGDB did not expose the values of system registers;
2.  KGDB provided no way to directly access the physical memory.

Nevertheless, I decided to go down the rabbit hole and figure out how I can make GEF work. This required changes on both the KGDB kernel module and the GEF side. I will not describe all the details here and will just mention the core points.

**Exposing system registers**. To deal with the first issue, I looked through the KGDB kernel module code and found a way to expose selected system registers.

First, I extended the number of register values the KGDB module sends to GDB:

```c
diff --git a/arch/arm64/include/asm/kgdb.h b/arch/arm64/include/asm/kgdb.h
index 21fc85e9d2be..68ca7caaa5c7 100644
--- a/arch/arm64/include/asm/kgdb.h
+++ b/arch/arm64/include/asm/kgdb.h
@@ -75,12 +75,13 @@ extern int kgdb_fault_expected;
 #define _GP_REGS        33
 #define _FP_REGS        32
...
@@ -97,6 +98,6 @@ extern int kgdb_fault_expected;
  */
...
And then updated `dbg_get_reg` to include the values of the new registers:

```c
diff --git a/arch/arm64/kernel/kgdb.c b/arch/arm64/kernel/kgdb.c
index 4e1f983df3d1..c5011293e989 100644
--- a/arch/arm64/kernel/kgdb.c
+++ b/arch/arm64/kernel/kgdb.c
@@ -99,6 +99,14 @@ struct dbg_reg_def_t dbg_reg_def[DBG_MAX_REG_NUM] = {
     { "v31", 16, -1 },
     { "fpsr", 4, -1 },
     { "fpcr", 4, -1 },
+    // Registers required by bata24/gef; values filled dynamically:
+    { "TTBR0_EL1", 8, -1 },
+    { "TTBR1_EL1", 8, -1 },
+    { "TCR_EL1", 8, -1 },
+    { "SCTLR_EL1", 8, -1 },
...
@@ -111,6 +119,45 @@ char *dbg_get_reg(int regno, void *mem, struct pt_regs *regs)
...
+        memcpy(mem, (void *)&ttbr0, dbg_reg_def[regno].size);
+        break;
+    }
+    case 69: { // TTBR1_EL1
+        unsigned long ttbr1 = read_sysreg(ttbr1_el1);
+        memcpy(mem, (void *)&ttbr1, dbg_reg_def[regno].size);
+        break;
+    }
+    case 70: { // TCR_EL1
+        unsigned long tcr = read_sysreg(tcr_el1);
+        memcpy(mem, (void *)&tcr, dbg_reg_def[regno].size);
+        break;
+    }
+    case 71: { // SCTLR_EL1
+        unsigned long sctlr = read_sysreg(sctlr_el1);
+        memcpy(mem, (void *)&sctlr, dbg_reg_def[regno].size);
+        break;
+    }
+    case 72: { // ID_AA64MMFR1_EL1
+        unsigned long mmfr1 = read_sysreg_s(SYS_ID_AA64MMFR1_EL1);
+        memcpy(mem, (void *)&mmfr1, dbg_reg_def[regno].size);
+        break;
+    }
+    case 73: { // ID_AA64MMFR2_EL1
+        unsigned long mmfr2 = read_sysreg_s(SYS_ID_AA64MMFR2_EL1);
+        memcpy(mem, (void *)&mmfr2, dbg_reg_def[regno].size);
+        break;
+    }
+    case 74: { // VBAR_EL1
+        unsigned long vbar = read_sysreg(vbar_el1);
+        memcpy(mem, (void *)&vbar, dbg_reg_def[regno].size);
+        break;
+    }
+    }
+
...
**GDB target description**. Now, I had to tell GDB that the KGDB module sends more register values than expected by default.

For this, I created a custom target description file `xairy.xml`:
...
<target version="1.0" xmlns:xi="http://www.w3.org/2001/XInclude">
  ...
  <!-- Standard AArch64 register sets -->
...
And loaded it into GDB via `set tdesc filename xairy.xml`.

`aarch64-core.xml` and `aarch64-fpu.xml` in the snippet above contain the descriptions of the ARM64 registers GDB knows about. You can get these files from the [QEMU repository](https://github.com/qemu/qemu/tree/master/gdbstub/gdb-xml). Just put them next to `xairy.xml` before running `set tdesc`.

**Registers exposed**. Now, I could see the system register values in GDB:

```bash
gef> info r
...
TTBR0_EL1      0x81c0008dc833001   0x81c0008dc833001
TTBR1_EL1      0x81c000081c46001   0x81c000081c46001
TCR_EL1        0x55001f2b5593519   0x55001f2b5593519
SCTLR_EL1      0x20000383474d99d   0x20000383474d99d
ID_AA64MMFR1_EL1 0x11312122        0x11312122
ID_AA64MMFR2_EL1 0x1221011110101011 0x1221011110101011
VBAR_EL1       0xffffffc008010800  0xffffffc008010800
```

**Accessing physical memory**. The second issue was that KGDB did not implement commands to directly access the physical memory.

However, the Linux/Android kernel has physmap (aka the direct linear mapping) — a special region of memory that maps all (most of) the physical memory byte-to-byte. So instead of relying on GDB commands to read the physical memory, GEF could just get the values from physmap.

I [filed](https://github.com/bata24/gef/issues/151) an issue to GEF that described this idea and bata24 quickly [implemented](https://github.com/bata24/gef/commit/864418d02a4c09a49b48390974d1c97bd2e5ea1b) a solution.

**Assorted GEF fixes**. Besides the mentioned changes, GEF required a number of other fixes to make `slub-dump` (and other commands) work reasonably well in the KGDB mode:

-   Various performance optimizations (e.g., getting symbol addresses and structure layouts from the `vmlinux` debug information);
-   Avoiding walking the page tables when it’s not required (walking takes quite long over KGDB);
-   Adding support for forcing the KGDB mode when GEF cannot auto-detect it (e.g., when using `agent-proxy`);
-   [Fixing](https://github.com/bata24/gef/issues/143) `buddy-dump` to account for the unusual value of `MIGRATE_PCPTYPES` on Android.

You can look through the [GEF issues](https://github.com/bata24/gef/issues?q=is%3Aissue%20author%3Axairy) I reported (and the fixes that bata24 implemented for them) and the [changes](https://github.com/bata24/gef/commits?author=xairy) I implemented myself.

As a side-effect of all these changes, `slub-dump` and `buddy-dump` became much faster while debugging a `vmlinux` with debug symbols even when not using KGDB.

**Important**. The KGDB mode support in GEF has been slightly [reworked](https://github.com/bata24/gef/pull/157#issuecomment-3955304484) recently, and I have not tested the changes yet. The last commit that I tested and that is known to work is [1b72f3c4b97](https://github.com/bata24/gef/commit/1b72f3c4b97fa4d86e8d9dc109b286ef3098b7a4).

**GEF works**. With these changes in place, the `slub-dump`, `buddy-dump`, `vmalloc-dump`, `pagewalk`, and some other commands now work in the KGDB mode.
...
`gef.kgdb_force` forces the KGB mode; required when using `agent-proxy`. `gef.kgdb_system_registers` tells GEF that KGDB exposes the system register values.

I suspect many other GEF commands can be allowed and will work in the KGDB mode. However, the commands that require walking the page tables or scanning memory will take a significant time to complete, as KGDB is slow.

I would like to thank bata24 for quickly reacting to all the reported issues and implementing the fixes! Amazing job! 💜

## 📝 Summary and afterword

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/03e4a11eecc5cd8d.jpg)
