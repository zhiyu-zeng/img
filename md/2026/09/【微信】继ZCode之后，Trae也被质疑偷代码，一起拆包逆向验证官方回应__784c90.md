---
title: 【微信】继ZCode之后，Trae也被质疑偷代码，一起拆包逆向验证官方回应
source: https://mp.weixin.qq.com/s/vI6xvY6FLfGjTJrAKiIS_Q
source_host: mp.weixin.qq.com
clip_date: 2026-09-23T18:30:42+08:00
trace_id: ba7f244c-ad63-4c6d-999d-0fb8438d1ae0
content_hash: 895361dcfdeb3a0261a913f59633db7e378a2e3f2fd0dd349afc85b72b5e8c4a
status: synced
tags:
  - 微信
  - Windows逆向
...
ai_summary: 逆向拆包验证Trae 3.3.104：官方三点回应基本属实，不存在ZCode式Git历史外传，但遥测链路繁多、远程embedding受服务端开关控制。
ai_summary_style: key-points
images_status:
  total: 5
  succeeded: 4
  failed_urls:
    - https://mmbiz.qpic.cn/sz_mmbiz_png/rapaL0gDxQrASyib015LD5dyRibFhnkhmeByDibeno06tDAZSVljQHvEZCVYsO7S8ib0mHLZQb9Nicqg2FzVibSkhZSnEI6g8zrXRib4UHI7n8Ty4U/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=1
notion_page_id: 3e475244-d011-81fd-9e77-d5400531e08b
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 逆向拆包验证Trae 3.3.104：官方三点回应基本属实，不存在ZCode式Git历史外传，但遥测链路繁多、远程embedding受服务端开关控制。
> 
> - **对比基准：** ZCode事件中被上传数据86.6%来自.git目录，含提交历史、LFS对象与reflog。
> - **默认本地：** ckg模块（libckg.dll，Go编译、符号未裁剪）内确有远端上传端点，但main.js第911行硬编码`local_embedding:!0`，向量库落盘sqlite_vec，实测代码明文不出本机。
> - **过滤机制：** 二进制内嵌go-gitignore与TCC两套规则检查器，.git/、node_modules、构建产物、二进制与媒体文件默认排除，与ZCode形成关键区别。
> - **企业扩展：** tob-git-ai的checkpoint仅上报元数据（仓库路径、commit、模型名、补丁长度），上限200变更/256KB单补丁/2MB总量，且取不到account.tenant_id即不启用，个人版不加载。
> - **遗留风险：** manifest.json明文声明设备注册、崩溃、监控、分析（mcs.zijieapi.com）等多条遥测链路；安装器采集SMBIOS；隐私模式只管训练用途、不覆盖索引，易致用户误解。

**星宇Sec** *2026年9月23日 18:09*
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/91c622bbd39917c6.png)
...
二、索引上传范围有严格过滤机制：依赖目录（node_modules等）、构建产物、二进制文件、图片、音视频、压缩包均不上传；以.开头的隐藏目录（包括.git）不上传；.gitignore中列出的文件默认忽略，用户可在.trae/.ignore中自定义排除规则；单文件大小与行数设有上限；

三、隐私模式控制的是对话内容是否用于数据分析与模型训练，与代码索引功能无关。官方文档明确："无论隐私模式是否开启，TRAE绝不会将你的代码库文件用于数据分析、产品优化或模型训练。"

![⚠️ 图片托管失败 · 图片](https://mmbiz.qpic.cn/sz_mmbiz_png/rapaL0gDxQrASyib015LD5dyRibFhnkhmeByDibeno06tDAZSVljQHvEZCVYsO7S8ib0mHLZQb9Nicqg2FzVibSkhZSnEI6g8zrXRib4UHI7n8Ty4U/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=1)
...
其中Embedding（向量化）、SplitFile（文件切分）、CodeChunk（代码块）等类型与官方对索引功能的描述一一对应；结构体标签upload_knowledgebase_files_batch_size则表明批量上传配置确有实现。同一二进制中提取到的远端端点字符串证实，远程上传链路在产品中真实存在：
...
实测运行后，ckg进程启动日志打印的配置为"local_embedding":true、"embedding_storage_type":"sqlite_vec"，向量库落盘路径为本机AppData\\Roaming\\Trae CN\\ModularData\\ckg_server。在实测的3.3.104版本中，索引默认在本地完成向量化，代码明文不出本机。远程embedding模式受服务端功能开关（enable_local_embedding）控制，默认未启用。
...
![图1\_ckg代码索引数据流](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/1f533ad16a48d856.jpg)
...
即取不到account.tenant_id时直接返回null，扩展不启用；extension.js中的运行时注释亦说明其"daemon home与投递队列数据库由启用时解析的租户ID派生"，属企业租户专属能力，个人版不加载。此外，ai-completion扩展中的gitDiff逻辑仅对当前打开文件执行git diff

![图3：checkpoint上报字段裁剪](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/0cc56439125f0f34.jpg)
...
![图2：Trae运行期网络行为总览](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/ecb4c0138564f935.jpg)
...
即设备注册（log.snssdk.com）、崩溃上报（crash.snssdk.com）、监控上报（pc-mon.zijieapi.com）、远程配置（tnc3-bjlgy.zijieapi.com）等域名均在配置中静态声明，与运行时流量互相印证。安装器亦采集SMBIOS硬件信息（debug.log含read_smbios记录）。此类遥测属字节系产品通用方案，与"搬运Git历史"性质不同，但隐私敏感用户有理由关注。
