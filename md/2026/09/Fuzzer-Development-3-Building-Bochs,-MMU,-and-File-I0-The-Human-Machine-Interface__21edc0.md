---
title: "Fuzzer Development 3: Building Bochs, MMU, and File I/0 | The Human Machine Interface"
source: https://h0mbre.github.io/Loading_Bochs/
source_host: h0mbre.github.io
clip_date: 2026-09-09T00:05:10+08:00
trace_id: 5f9b051d-a123-41fe-8549-c4e659e30eee
content_hash: 7f20eb9cc9b0cb3617f80e4573509db3bb0e716685c7a0046c618296d9aff2bd
status: synced
tags:
  - 快照Fuzzer
  - Bochs
series: null
feed_source: h0mbre
ai_summary: Lucid 快照 Fuzzer 已能加载 Bochs：通过 Musl 直调 Rust 简化系统调用沙箱，加入 Fault 回传、TLS 模拟、简易 MMU 与文件 I/O 仿真。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3d575244-d011-81a5-be53-ebeac94f31a0
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> Lucid 快照 Fuzzer 已能加载 Bochs：通过 Musl 直调 Rust 简化系统调用沙箱，加入 Fault 回传、TLS 模拟、简易 MMU 与文件 I/O 仿真。
> 
> - **系统调用简化：** 放弃完整上下文切换/C ABI 转 Syscall ABI 寄存器搬运，Musl 的 `__syscall6` 在 Lucid 环境下直接调用 Rust 函数 `lucid_syscall`，大幅降低复杂度。
> - **上下文切换重构：** 只传 `LucidContext` 指针，由 `context_switch` 基于 `mode` 字段决定保存 Bochs 或 Lucid 寄存器；为此硬编码少量结构体偏移，并把 Fault 通过执行上下文回传给 Lucid 处理。
> - **TLS 沙箱：** 定位到 Musl 的 `__set_thread_area` 会用内联 syscall 直改 `fs` 绕过拦截；改为走受控 `syscall()` 包装器，并在 Lucid 上下文中提供自有 TLS 块（含 errno），便于 syscall 处理器设置错误号。
> - **Bochs 构建：** 使用 `musl-cross-make` 从零构建基于自定义 Musl 的完整工具链（含 libstdc++），对 Musl 打补丁后以 `--static-pie` 编译 Bochs，启用 x86-64、VMX、AVX 等特性。
> - **MMU 与文件 I/O：** 预先为 brk 和 mmap 分配两块内存池，当前只按偏移分配、不回收不释放；处理 brk/mmap/munmap，支持普通匿名映射和 brk 内存上的 `MAP_FIXED`。文件则预读进 `FileTable`，通过 fd 和游标模拟 open/read。
