---
title: ARM64 Trace 检测
source: https://yuuki.cool/posts/dbidetect/dbi_detect/
source_host: yuuki.cool
clip_date: 2026-09-07T13:19:03+08:00
trace_id: 5aab78ce-94f6-468c-a5a7-22e25c309774
content_hash: e43f7ab6c656e2a26290a5a8e076dec24e0958dc29142f34db7f168bfa940841
status: synced
tags:
  - 反调试
  - Android逆向
...
ai_summary: ARM64下可用信号上下文和BRK异常现场读取Physical PC，检测DBI trace；异步采样和BRK两种方式结合能区分原生执行与code cache执行。
ai_summary_style: key-points
images_status:
  total: 2
  succeeded: 0
  failed_urls:
    - dbi-execution-flow.svg
    - dbi-detection-flow.svg
notion_page_id: 3d475244-d011-8132-af22-e94ea75f79c1
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> ARM64下可用信号上下文和BRK异常现场读取Physical PC，检测DBI trace；异步采样和BRK两种方式结合能区分原生执行与code cache执行。
> 
> - **原理：** DBI通过JIT重编译basic block并写入code cache，形成Guest PC与Physical PC两套地址；ADR指令会被DBI翻译并维持Guest语义，所以不能作为可靠的观察点。
> - **异步采样：** 探针内用ADR记录async_begin/async_end，watchdog线程通过pthread_kill逐次发SIGUSR2（非实时信号会合并），从ucontext_t读取被中断时的PC，共采8个样本；任一样本落在探针窗口外即强证据。
> - **BRK异常：** 探针先记录brk #0x4d2标签地址，执行后由SIGTRAP handler校验ucontext_t.pc；若DBI直接让异常落在code cache，PC会指向非预期地址，若DBI模拟流程绕过异常，则trap_seen为0。
> - **结果判定：** 任一强证据都会置进程级sticky状态，后续检查持续返回已检测；实测原生执行时样本全部在窗口内且SIGTRAP PC等于BRK地址，开启trace后两条检测路径均观察到范围外PC。
> - **落地建议：** 可将整个业务函数放入VM执行，命中时返回不同算法结果（如SHA-256前32位），后端验签可感知并标记，从而提升AI trace还原成本。
...
![⚠️ 图片托管失败 · 原生执行与DBI执行路径](dbi-execution-flow.svg)

![⚠️ 图片托管失败 · 原生执行与DBI执行路径](https://yuuki.cool/posts/dbidetect/dbi_detect/dbi-execution-flow.svg)
...
![⚠️ 图片托管失败 · DBI Trace检测流程](dbi-detection-flow.svg)

![⚠️ 图片托管失败 · DBI Trace检测流程](https://yuuki.cool/posts/dbidetect/dbi_detect/dbi-detection-flow.svg)
...
<video src="/media/DbiDetect/show.mp4" controls preload="metadata" style="width: 100%; border-radius: 0.75rem;"></video>
