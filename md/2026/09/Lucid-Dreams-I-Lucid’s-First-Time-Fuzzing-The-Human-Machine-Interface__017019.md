---
title: "Lucid Dreams I: Lucid’s First Time Fuzzing | The Human Machine Interface"
source: https://h0mbre.github.io/Lucid_Dreams_1/
source_host: h0mbre.github.io
clip_date: 2026-09-09T00:08:26+08:00
trace_id: 1efe0e2b-90c2-48d3-bddd-d2ee29ad5509
content_hash: 8ca2339a9ea0cf540ddaa55d3aa036b24652d74c9fcc9ff429c709d689460573
status: synced
tags:
  - 内核
  - 模拟执行
series: null
feed_source: h0mbre
ai_summary: Lucid 全系统快照模糊测试器首次实战落地：改进差分快照恢复、自建内核 Netlink harness，并借助高效种子与修复后的 Redqueen，将边覆盖率从约 1.7 万提升到 2.6 万。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3d575244-d011-81c3-948f-cdbebb28097b
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> Lucid 全系统快照模糊测试器首次实战落地：改进差分快照恢复、自建内核 Netlink harness，并借助高效种子与修复后的 Redqueen，将边覆盖率从约 1.7 万提升到 2.6 万。
> 
> - **快照方案升级：**旧方法通过 mmap 内存备份恢复全部可写内存，多核下扩展性急剧劣化；新方案把 Bochs 可写页设为 PROT_READ，写入触发缺页后记录脏页，重置时仅用 memcpy 差分恢复脏页，热路径无 syscall，接近 1:1 的线性扩展。
> - **Harness 设计：**新增 lucid_fuzz 内核测试 syscall，绕过用户态/内核态切换，直接将输入解析为最多 16 条 Netlink 消息，按协议（NETLINK_ROUTE、XFRM、NETFILTER、CRYPTO）预分配 skb 并派发给对应接收 handler；解析逻辑极严格，任何越界或剩余字节都会导致超时。
> - **分阶段实测对比：**纯字节变异 22 小时 88.3M 次迭代仅发现 16917 条边；加入修正 nlmsg_len/type/flags 等策略后，2.8M 次迭代即获 17740 条边，效率提升约 35 倍；未修复的 Redqueen 实现曾让 90.9% CPU 时间消耗在分析上（370K 次迭代/14 小时）。
> - **Redqueen 修复与种子阶段：**修复了编码展开过泛、未按 RIP 去重、预生成海量补丁输入导致被 SIGKILL 等问题；之后 Redqueen 在单次分析中为 fuzzer-4 新增 1555 条边；再用 LD_PRELOAD hook sendmsg 从 tc/nft 等真实工具抓取约 30 个种子，最终 15.8 小时、192 万次迭代刷新至 25866 条边（覆盖率 39.47%）。
> - **结论与经验：**Bochs 模拟是主要瓶颈（约比原生慢百倍）；高质量种子是提升效率最显著的单项改动；语料去膨胀、偏向新输入等优化对结果影响不大；全部运行 0 crashes，唯一 OOB 读因绕过上游检查而不计入漏洞。
