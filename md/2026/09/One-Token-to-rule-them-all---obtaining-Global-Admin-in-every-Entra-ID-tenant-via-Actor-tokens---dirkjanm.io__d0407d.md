---
title: One Token to rule them all - obtaining Global Admin in every Entra ID tenant via Actor tokens - dirkjanm.io
source: https://dirkjanm.io/obtaining-global-admin-in-every-entra-id-tenant-with-actor-tokens/
source_host: dirkjanm.io
clip_date: 2026-09-25T10:18:10+08:00
trace_id: 1fd96194-a745-418e-baf2-e20be0d1d289
content_hash: 4c4a660944e1ee42f31f78591c483e39cc95c7c43d8501eab3ff8647be27efe6
status: synced
tags:
  - 漏洞分析
  - 协议分析
...
ai_summary: Azure AD Graph API 未校验来源租户，使 Actor 令牌可冒充任意租户的全局管理员；微软数日内修复并分配 CVE-2025-55241。
ai_summary_style: key-points
images_status:
  total: 9
  succeeded: 9
  failed_urls: []
notion_page_id: 3e675244-d011-8148-bfaf-f9274ec8b5b9
ioc:
  cves:
    - CVE-2025-55241
  cwes: []
...
> 💡 **AI 总结（key-points）**
>
> Azure AD Graph API 未校验来源租户，使 Actor 令牌可冒充任意租户的全局管理员；微软数日内修复并分配 CVE-2025-55241。
> 
> - **令牌特性：** Actor 令牌由 Access Control Service 签发，有效期 24 小时，含 `trustedfordelegation=true`；配套冒充令牌是 `alg=none` 无签名 JWT，服务端可自行伪造，签发与使用均无日志、无法吊销，完全绕过条件访问。
> - **致命缺陷：** 把冒充令牌的租户 ID 改成他人租户后 API 仍接受，只需公开的租户 ID 加一个用户 `netId` 即可读取数据；`netId` 递增生成，可暴力枚举，耗时分钟到小时级且不留痕。
> - **攻击链：** 按域名解析租户 ID → 取得普通用户 `netId` → 冒充并列出全局管理员 → 冒充其读写，可新建账号授予管理员，或给应用加凭据以窃取邮件与文件。
> - **B2B 跳跃：** 读取受邀客账号 `alternativeSecurityIds` 中的 `netId`，即可反向冒充该用户在其主租户；每租户仅 2 次无日志 API 调用，可指数级扩散，首个常是微软自身租户。
> - **检测与披露：** 查询无日志、仅修改留痕，日志呈现"Exchange 显示名 + 被冒充管理员 UPN"的异常组合，官方给出 KQL 检测；7 月 14 日报 MSRC，7 月 17 日全局修复，9 月 4 日发布 CVE-2025-55241。
...
![Changed tenant ID](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/a01ef2fd404be6b9.png)
...
![Error message indicating the user does not exist](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/b6f1bc5c9427a3d4.png)
...
![Craft impersonation token for Guest user](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/3ea104b44868a22e.png)

I use this token to query the `netId` of a Global Admin:

![Query Global Admin](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/6e294f14798a9075.png)

Then I create an impersonation token for this Global Admin (the UPN is kept the same since it is not validated by the API):

![Craft impersonation token for Global Admin](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/acfa3e7e76010cfb.png)
...
![Query data in the tenant](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/e577cc96e42d4773.png)
...
![Running roadrecon in a tenant with an impersonation token](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/9397736bf765e069.png)
...
![Guest user link based on netid](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/54d192407c1f1c2b.png)
...
![Initiated by exchange and a global admin](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/09/89772d0876e36b3d.png)
