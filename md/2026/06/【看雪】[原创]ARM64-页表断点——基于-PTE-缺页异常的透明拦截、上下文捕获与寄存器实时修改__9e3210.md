...
clip_date: 2026-06-15T21:16:10+08:00
trace_id: 124ddb5f-bcf2-46a5-a1f2-af11077f47b1
content_hash: 654e7e8df07757aee8bdf0d34fef4f090a37cce87f5243964c85df6391e91074
status: summarized
tags:
  - 看雪
series: null
ai_summary: 通过修改 ARM64 PTE 权限位制造缺页异常，在内核拦截处理中捕获用户态上下文并实时修改，实现完全透明的指令级调试。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 38075244-d011-8181-9d7d-ef188688b15f
---

> 💡 **AI 总结（key-points）**
>
> 通过修改 ARM64 PTE 权限位制造缺页异常，在内核拦截处理中捕获用户态上下文并实时修改，实现完全透明的指令级调试。
> 
> - **技术原理：** 通过修改目标地址的 PTE 权限位（如设置 UXN 位禁止执行），触发 CPU 缺页异常。在 `do_page_fault` 的 hook 中，可读取并修改完整的寄存器上下文（GPR x0-x7、SIMD v0-v31、栈参数），随后恢复原始 PTE 并允许 CPU 重试指令，过程对用户态透明。
> 
> - **版本演进：** 项目从 **V1 阻塞模式**（命中后等待用户态读取并修改寄存器）发展到 **V2 纯非阻塞 one-shot 模式**（仅记录快照），最终到 **V3 非阻塞 + 寄存器修改**（用户预先设置修改数据，命中时内核直接应用）。
> 
> - **性能与规模：** 单次缺页处理延迟约 **5 微秒**；支持最多 **32 个全局断点槽位**，但 **每个 4KB 页只允许一个断点**；兼容 3 级（39-bit VA）和 4 级（48-bit VA）页表。
> 
> - **核心限制：** 仅支持 **EL0 用户态异常**，不支持内核态；对 **2MB 大页代码段设置执行断点** 会影响整个范围；断点功能依赖特定内核符号（如 `do_page_fault`）地址，可能与内核版本相关。
