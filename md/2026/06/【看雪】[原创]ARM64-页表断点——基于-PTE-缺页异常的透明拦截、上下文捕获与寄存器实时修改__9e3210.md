---
title: 【看雪】[原创]ARM64 页表断点——基于 PTE 缺页异常的透明拦截、上下文捕获与寄存器实时修改
source: https://bbs.kanxue.com/thread-291616.htm
source_host: bbs.kanxue.com
clip_date: 2026-06-15T18:35:37+08:00
trace_id: c30dc057-44f4-4213-ab7f-cd41528b768a
content_hash: 1e4d38fbbfba5ffb7b378a416298a3ea18bfc6fc7307202ea6a1379151e805fb
status: summarized
tags:
  - 看雪
series: null
ai_summary: 基于PTE权限位修改，通过制造可控缺页异常，实现对ARM64用户态进程的透明指令级拦截、上下文捕获及寄存器修改。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 38075244-d011-8171-9d66-c85acc3359bf
---

> 💡 **AI 总结（key-points）**
>
> 基于PTE权限位修改，通过制造可控缺页异常，实现对ARM64用户态进程的透明指令级拦截、上下文捕获及寄存器修改。
> 
> - **透明性原理：** 通过修改目标地址PTE的权限位（如`PTE_UXN`）触发CPU异常，在`do_page_fault`的hook中处理后立即恢复原始PTE，对目标进程完全不可见（无信号、VMA不变、不检测到）。
> - **上下文捕获能力：** 断点命中时，可一次性捕获调用时的全部参数，包括通用寄存器x0-x7、SIMD寄存器v0-v31（通过内联汇编`stp`指令读取）以及栈参数。
> - **实时寄存器修改：** 支持在断点命中时实时修改GPR、SIMD寄存器及SP。用户态可预先设置修改数据，内核在异常处理中应用，并可支持实时更新修改值。
> - **关键实现步骤：** 手动遍历目标进程的四级页表（PGD->PTE）并直接操作物理内存；通过内联汇编实现对FPSIMD寄存器的读写；使用`compiler_barrier`和`mod_ready`标志保证修改数据的线程安全；引入Sticky Page机制防止同页内无限缺页循环。
> - **局限与检测风险：** 受限于页级控制，同一4KB页仅支持一个断点，不支持1GB大页映射。存在被检测可能，如通过执行时间差测量、内存数据双重校验（shadow copy）或环境完整性扫描（针对注入框架本身）。
...
### 5.2 do\_page\_fault Inline Hook
...
**mod\_ready 标志** ：用户态可能随时调用 `PRCTL_BP_MODIFY` 更新修改数据，与 `do_page_fault` 形成竞态。
...
完整的代码和使用指南见 [user-test/](https://bbs.kanxue.com/user-test/) 和 [kpms/prctlhookRWMemoryNew/](https://bbs.kanxue.com/kpms/prctlhookRWMemoryNew/) 。
...
| 检测手段 | 风险  | 本项目暴露面 |
| --- | --- | --- |
| 值域校验 (边界/NaN) | **中** | v\_mode 选错致高位垃圾，或值超出合法范围 |
...
6.  **v\_mode 选对** ：LO32 写 float 时确保高位干净；Q128 时用完整 16 字节
...
[#基础理论](https://bbs.kanxue.com/forum-161-1-117.htm) [#源码框架](https://bbs.kanxue.com/forum-161-1-127.htm) [#系统相关](https://bbs.kanxue.com/forum-161-1-126.htm) [#HOOK注入](https://bbs.kanxue.com/forum-161-1-125.htm)
