---
title: 【看雪】AndProxy 升级版：基于 Seccomp 的无 Hook Binder 服务代理
source: https://bbs.kanxue.com/thread-291098.htm
source_host: bbs.kanxue.com
clip_date: 2026-06-16T17:06:48+08:00
trace_id: 124b02a9-431b-4375-93ed-340a8baba014
content_hash: 41647385d63d8ef1531eb9a966e36adc508809ec259c675629911b7b8b1fd9e4
status: summarized
tags:
  - 看雪
series: null
ai_summary: 通过 `seccomp-notify` 机制拦截 `ioctl` 系统调用，仅修改 Binder 事务中的 `target.handle` 字段，将调用透明路由至代理服务，实现完全无 Hook 的 Binder 服务代理。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 38175244-d011-811d-8b03-f83aa52ec170
---

> 💡 **AI 总结（key-points）**
>
> 通过 `seccomp-notify` 机制拦截 `ioctl` 系统调用，仅修改 Binder 事务中的 `target.handle` 字段，将调用透明路由至代理服务，实现完全无 Hook 的 Binder 服务代理。
> 
> - **核心机制：** 在 `ioctl(BINDER_WRITE_READ)` 系统调用层拦截并解析 `binder_transaction_data`，将其中的 `target.handle` 替换为代理服务的 handle，交由 Binder 驱动原生路由机制完成转发。
> - **方案优势：** 完全无侵入，不修改 GOT 表、代码段或 `libbinder.so`，避免了传统 Hook 的痕迹，且无需重新实现 Binder 协议栈。
> - **编译期处理：** 使用 `@ProxyMethod` 注解声明代理方法（需指定服务类名、服务名和方法名），KSP 处理器在编译期自动生成代理路由表和代理服务类。
> - **运行时架构：** 由三部分构成：KSP 编译期（生成路由表）、Seccomp 拦截层（在用户态 supervisor 中修改事务数据）、辅助系统（维护 handle 映射和启动代理服务）。
> - **局限与要求：** 需要 Linux 内核 5.10+ 和 Android 12+ 支持 `seccomp-notify`；存在 Binder 域隔离、并发处理复杂及二次 IPC 开销等挑战。
