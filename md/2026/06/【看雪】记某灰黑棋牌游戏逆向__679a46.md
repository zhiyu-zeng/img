...
clip_date: 2026-06-16T17:12:04+08:00
trace_id: e019bb79-1628-4e98-8d93-d69ee22c8344
content_hash: e679231c09c2045603f8be917c08813d3a5e0417f3716ccbdbcec88f628d985a
status: summarized
tags:
  - 看雪
series: null
ai_summary: 通过多层脱壳与魔改LuaJIT逆向，成功还原了棋牌游戏的HMAC-SHA1请求签名算法。
ai_summary_style: key-points
images_status:
  total: 12
  succeeded: 12
  failed_urls: []
notion_page_id: 38175244-d011-81d4-ba33-e13b13e2de6f
---

> 💡 **AI 总结（key-points）**
>
> 通过多层脱壳与魔改LuaJIT逆向，成功还原了棋牌游戏的HMAC-SHA1请求签名算法。
> 
> - **脱壳过程：** 应用有四层壳，核心是RC4+zlib解密获取真实业务DEX，以及通过Hook自定义Linker在内存中dump出未落地的、包含魔改LuaJIT虚拟机的子SO文件。
> - **Lua脚本提取：** 在运行时Hook LuaJIT的字符串创建函数`lj_str_new`或脚本加载函数，成功截获并保存了900余个已解密的Lua脚本文件。
> - **魔改opcode还原：** 魔改仅改变了操作码到处理器函数的映射表，指令编码不变。通过在IDA中分析分发循环和初始化代码，结合动态验证，恢复了完整的操作码映射表。
> - **反编译工具适配：** 修改了LuaJIT反编译器ljd的核心查表逻辑（`_MAP`），使其能正确解析魔改字节码，关键是不能覆盖指令对象中的标准操作码属性。
> - **签名算法复现：** 静态分析定位到`sign4Url`函数，但动态Hook `lj_str_new`抓取到真实HMAC素材后，才完全确认签名格式为 `HMAC-SHA1(密钥, timestamp|排序后查询串|timestamp|密钥|版本号|URL路径)`，并实现了字节级验证。
...
![image-20260614024212900](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/b6780d5a952bb79b.png)
...
![image-20260614024747609](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/d4816a034638dd65.png)
...
![image-20260614040945351](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/77cee98692c7fbf7.png) ![image-20260614041003084](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/ec037f59100e7b1b.png)
