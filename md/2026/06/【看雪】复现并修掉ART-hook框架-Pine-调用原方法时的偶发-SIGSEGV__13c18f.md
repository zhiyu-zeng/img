...
clip_date: 2026-06-18T12:49:54+08:00
trace_id: 05a32424-84c8-4ee0-a889-d5086f6cd895
content_hash: 75196f1394c9ab3df4e846c585923f3ce70ec60cfcb4d317e7c353d11284ecd6
status: summarized
tags:
  - 看雪
series: null
ai_summary: ART hook 框架 Pine 在调用被 hook 方法的原实现（backup 方法）时，由于移动 GC（特别是 Android 13+ 的 CMC）会搬动对象，导致游离的 backup 方法中未被跟踪的 `declaring_class` 引用变成野指针，在特定操作序列下触发偶发 SIGSEGV 崩溃；修复方案是在调用 backup 的关键窗口期临时禁用移动 GC。
ai_summary_style: key-points
images_status:
  total: 2
  succeeded: 0
  failed_urls:
    - https://blog-img-1393828675.cos.ap-shanghai.myqcloud.com/img/tine-gc-fig1-dangling.png
    - https://blog-img-1393828675.cos.ap-shanghai.myqcloud.com/img/tine-gc-fig2-timeline.png
notion_page_id: 38375244-d011-8157-ad3c-e4dd08da7086
---

> 💡 **AI 总结（key-points）**
>
> ART hook 框架 Pine 在调用被 hook 方法的原实现（backup 方法）时，由于移动 GC（特别是 Android 13+ 的 CMC）会搬动对象，导致游离的 backup 方法中未被跟踪的 `declaring_class` 引用变成野指针，在特定操作序列下触发偶发 SIGSEGV 崩溃；修复方案是在调用 backup 的关键窗口期临时禁用移动 GC。
> 
> - **根因分析：** backup 方法通过 `malloc` 分配，其内存不在 ART 托管堆上，因此其内部的 `declaring_class` GcRoot 无法被垃圾回收器扫描和修正。当移动式 GC 搬动了该引用指向的类对象后，backup 中的引用就变成了指向旧地址的野指针。
> - **确定性复现：** 在堆分配压力下反复调用 hook 方法的原实现，并在 `syncMethodInfo` 与 `backup.invoke` 之间的窗口期强制触发一次移动 GC（`Runtime.getRuntime().gc()`），即可 100% 稳定复现崩溃。
> - **修复核心：** 使用 ART 的 `IncrementDisableMovingGC` / `DecrementDisableMovingGC` 接口，在调用 backup 前临时禁用移动 GC（仅禁止搬动对象，不阻止回收），保护 “同步 `declaring_class` 地址” 到 “方法真正进入栈帧” 这一竞态窗口期。
> - **实现要点：** 通过解析 ART 运行时符号（`art::gc::Heap` 的函数指针和 `art::Runtime::instance_`）获取必要接口；若符号缺失或版本过低（如 Android 4.4 无移动 GC），修复逻辑会安全降级为无操作，不引入新风险。
> - **验证结果：** 修复版在同一测试环境（Pixel 6 Pro, Android 16）下跑满 20 万次调用未再崩溃，并通过了包含并发 GC 压力的自动化测试。修复适用于 Android 5.0 至 16。

Pine（ [canyie/pine](https://bbs.kanxue.com/elink@b78K9s2c8@1M7s2y4Q4x3@1q4Q4x3V1k6Q4x3V1k6Y4K9i4c8Z5N6h3u0Q4x3X3g2U0L8$3#2Q4x3V1k6U0j5h3&6&6K9h3g2Q4x3V1k6H3K9h3&6W2) ）是目前用得比较多的 ART 方法 hook 框架。它有一个老问题：调用被 hook 方法的 **原实现** 时，偶发 native `SIGSEGV` ，概率性、堆栈不固定、重启可能就好。上游源码在出事的那一行留了 FIXME，但一直没修：

```java
// FIXME: GC happens here (you can add Runtime.getRuntime().gc() to test) will crash backup calling
```

本文做三件事：把这个崩溃在真机上 **确定性复现** 、拿到崩溃栈、定位到具体那一次内存读；分析根因，并说明几条看起来能修、其实不行的路；给出修法，换上修复版重新跑、拿到不崩的日志。修复已合入 Pine 的 fork（ [taisuii/tine](https://bbs.kanxue.com/elink@731K9s2c8@1M7s2y4Q4x3@1q4Q4x3V1k6Q4x3V1k6Y4K9i4c8Z5N6h3u0Q4x3X3g2U0L8$3#2Q4x3V1k6@1j5h3W2K6N6h3W2A6i4K6u0r3N6r3W2F1k6b7%60.%60.) ）。
...
![图1：移动 GC 如何让 backup 的 declaring\_class 变成野指针](⚠️ https://blog-img-1393828675.cos.ap-shanghai.myqcloud.com/img/tine-gc-fig1-dangling.png)
...
![图2：崩溃窗口 vs 修复，一次 backup 调用的时序](⚠️ https://blog-img-1393828675.cos.ap-shanghai.myqcloud.com/img/tine-gc-fig2-timeline.png)
...
上游框架： [canyie/pine](https://bbs.kanxue.com/elink@0c9K9s2c8@1M7s2y4Q4x3@1q4Q4x3V1k6Q4x3V1k6Y4K9i4c8Z5N6h3u0Q4x3X3g2U0L8$3#2Q4x3V1k6U0j5h3&6&6K9h3g2Q4x3V1k6H3K9h3&6W2) 。本文复现工程与修复在其 fork [taisuii/tine](https://bbs.kanxue.com/elink@00cK9s2c8@1M7s2y4Q4x3@1q4Q4x3V1k6Q4x3V1k6Y4K9i4c8Z5N6h3u0Q4x3X3g2U0L8$3#2Q4x3V1k6@1j5h3W2K6N6h3W2A6i4K6u0r3N6r3W2F1k6b7%60.%60.) 。
