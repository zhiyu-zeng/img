---
title: 【看雪】ARM64 页表断点——基于 PTE 缺页异常的透明拦截、上下文捕获与寄存器实时修改
source: https://bbs.kanxue.com/thread-291616.htm
source_host: bbs.kanxue.com
clip_date: 2026-06-16T17:13:22+08:00
trace_id: 55d50a47-492a-4800-a128-3e5c9b56f8a5
content_hash: c5ae2d4039e0751e1eead1603729a2ff73367070d10e8300cb62221b327954a3
status: summarized
tags:
  - 看雪
series: null
ai_summary: 本文提出一种基于ARM64 PTE权限位操控的透明断点技术，通过修改页表条目制造缺页异常，在内核中拦截用户态进程，实现参数捕获和实时修改，且对进程完全不可见。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 38175244-d011-8185-ab74-e9c3c2da68ac
---

> 💡 **AI 总结（key-points）**
>
> 本文提出一种基于ARM64 PTE权限位操控的透明断点技术，通过修改页表条目制造缺页异常，在内核中拦截用户态进程，实现参数捕获和实时修改，且对进程完全不可见。
> 
> - **透明拦截机制**：通过修改PTE权限位（如设置UXN、清除USER、设置RDONLY）触发CPU异常，在`do_page_fault`中捕获上下文后恢复PTE，避免触发SIGSEGV或修改VMA，进程无感知。
> - **断点类型与原理**：支持执行、读、写三种断点类型，对应设置PTE特定位引发不同的异常类（如Instruction Abort或Data Abort），通过ESR寄存器区分访问类型。
> - **上下文捕获与修改**：断点命中时捕获x0-x7、q0-q31寄存器和栈参数；支持实时修改GPR和SIMD寄存器，修改通过`pt_regs`写回或内联汇编实现。
> - **版本迭代与优化**：项目从V1阻塞模式演进到V2纯one-shot非阻塞，再发展到V3非阻塞加寄存器修改，引入Sticky Page机制避免无限缺页循环，提升性能。
> - **性能与限制**：单次断点处理延迟约5μs，支持最多32个断点槽位，每页限一个断点；仅适用于用户态，不支持大页（2MB）的执行断点。
