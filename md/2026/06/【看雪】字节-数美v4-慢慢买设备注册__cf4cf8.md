...
clip_date: 2026-06-29T10:27:03+08:00
trace_id: 0f8ee854-82e9-4c7b-a7f2-93338b19f309
content_hash: 98065af23491b1de5439aa81b2171928efc214d12ca742f7756e0dd6e2de692d
status: summarized
tags:
  - 看雪
series: null
feed_source: null
ai_summary: 要绕过“慢慢买”APP的字节、数美v4及自身业务三重设备注册风控，需模拟完整的设备注册流程并动态生成设备指纹。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 38e75244-d011-817b-bfa5-f20c9b3a68bb
ioc:
  cves: []
  cwes: []
  hashes:
...
> 💡 **AI 总结（key-points）**
>
> 要绕过“慢慢买”APP的字节、数美v4及自身业务三重设备注册风控，需模拟完整的设备注册流程并动态生成设备指纹。
> 
> - **风控绕过策略：** 需依次完成数美v4设备注册、字节设备注册、慢慢买业务设备注册三步，以获取必需的凭证（如`sm_deviceid`、`bd_did`、`mmbDevId`）。
> - **设备指纹随机化：** 通过模板文件生成随机设备指纹，涉及时间戳、电池状态、触摸性能等数十个参数，并进行Base64编码、压缩和加密处理以符合数美接口要求。
> - **加解密与请求封装：** 业务接口请求需对参数进行排序、盐值拼接、MD5签名生成token，数据体采用DES或AES加密；字节设备注册请求则需gzip压缩后进行特定加密。
> - **业务接口调用：** 使用注册获得的设备凭证，拼接包含设备ID、会话信息等的通用参数，调用具体业务接口（如获取商品优惠券）。
> - **设备复用限制：** 注册的设备会很快失效导致风控，文中代码策略是为每次业务请求都注册一套新设备，但理论上设备可复用一到两次。
