...
clip_date: 2026-06-16T17:07:49+08:00
trace_id: 79d69d86-ce02-4343-bf0d-71f9cfa5b632
content_hash: a3e2015eef7e4b694f789924e218103d1971e87c982803495f3d35ea01694d20
status: summarized
tags:
  - 看雪
series: null
ai_summary: Google Integrity校验并非绝对可靠，存在中间人攻击漏洞，攻击者可在Root且未解锁Bootloader的设备上导出证书链进行绕过。
ai_summary_style: key-points
images_status:
  total: 10
  succeeded: 10
  failed_urls: []
notion_page_id: 38175244-d011-81ca-8e75-e11507fe97d7
---

> 💡 **AI 总结（key-points）**
>
> Google Integrity校验并非绝对可靠，存在中间人攻击漏洞，攻击者可在Root且未解锁Bootloader的设备上导出证书链进行绕过。
> 
> - **生成流程**：Integrity Token请求通过Google Play服务发起，服务端收集设备信息、账号、包名及TEE证书链，经后端验证后返回Token。
> - **安全基础**：Key Attestation依赖TEE硬件生成的证书链，使用ECDSA算法签名验证，私钥不可导出，理论上保证完整性。
> - **漏洞条件**：设备需具备Root权限以导出证书链，且Bootloader未解锁以通过初始验证，满足这两点即可实施攻击。
> - **攻击方法**：通过Frida脚本拦截Key Attestation请求，将挑战值转发给另一台设备获取合法证书链，替换返回值，使未验证设备通过校验。
> - **实验结果**：在测试中，使用Root权限设备成功绕过Pixel 6的完整性验证，获得Google Integrity认证，证明机制存在缺陷。
