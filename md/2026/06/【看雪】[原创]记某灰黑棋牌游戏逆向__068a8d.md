...
trace_id: 57a25830-8728-4d5b-aac8-6c2b670d63ac
content_hash: daa2878fd20f9b4f94aeb34e7fa6611b9f18d713cc3562fcb304e98d8d710abb
status: summarized
tags:
  - 看雪
series: null
ai_summary: 通过脱壳、解密Lua脚本、破解魔改LuaJIT字节码并结合动态分析，成功还原了棋牌游戏的请求签名sign，其核心是HMAC-SHA1算法。
ai_summary_style: key-points
images_status:
  total: 4
  succeeded: 4
  failed_urls: []
notion_page_id: 38075244-d011-816c-94fe-c555dc501442
---

> 💡 **AI 总结（key-points）**
>
> 通过脱壳、解密Lua脚本、破解魔改LuaJIT字节码并结合动态分析，成功还原了棋牌游戏的请求签名sign，其核心是HMAC-SHA1算法。
> 
> - **脱壳过程：** 应用壳有四层，每层使用RC4+zlib加密，密钥通过包名/类名凯撒位移生成，可离线解密得到dex和子so文件。
> - **Lua脚本解密：** 脚本被XXTEA加密，通过hook `cocos2d::LuaStack::luaLoadBuffer` 在运行时解密并dump明文或LuaJIT字节码。
> - **魔改LuaJIT处理：** 字节码opcode编号被重排，通过静态分析分发循环和初始化代码还原映射表，并修改反编译器以保持标准opcode值。
> - **sign动态分析：** 静态反编译得到sign公式，但slot复用导致不确定性；动态hook `lj_str_new` 函数捕获HMAC输入字符串，确认标准HMAC-SHA1。
> - **密钥来源：** HMAC密钥和RC4密钥从图片隐写中读取，按"||"分割得到appid、HMAC_KEY、RC4_KEY和version四元组。
