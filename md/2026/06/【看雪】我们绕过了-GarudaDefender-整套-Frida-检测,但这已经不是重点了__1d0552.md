---
title: 【看雪】我们绕过了 GarudaDefender 整套 Frida 检测,但这已经不是重点了
source: https://bbs.kanxue.com/thread-291158.htm
source_host: bbs.kanxue.com
clip_date: 2026-06-05T13:42:15+08:00
trace_id: 45e690f0-3697-452e-8857-924888d7193c
content_hash: 5dd40e874106c871157e2f6f2a0c85e79e71aa43637837b0b44c12b6ab8c0947
status: synced
tags:
  - 看雪
  - Frida
  - AI辅助逆向
series: null
ai_summary: 作者绕过了GarudaDefender的Frida检测，但更重大发现是Hermes Agent能自主将分析过程沉淀为可复用方法论，预示逆向工程新范式。
ai_summary_style: key-points
images_status:
  total: 23
  succeeded: 23
  failed_urls: []
notion_page_id: 3ab75244-d011-8190-aeba-e671578264ea
---

> 💡 **AI 总结（key-points）**
>
> 作者绕过了GarudaDefender的Frida检测，但更重大发现是Hermes Agent能自主将分析过程沉淀为可复用方法论，预示逆向工程新范式。
> 
> - **绕过策略：** 成功静默了GarudaDefender的完整检测链路，包括Root、Hook及环境检测，核心是修改校验点并统一替换状态码。
> - **Agent自主沉淀：** Hermes Agent在分析中未经提示，自主创建了7个可执行的方法论Skill，用于定位告警入口、枚举检测任务等。
> - **方法论价值：** 生成的Skill是版本无关的行动指南，而非常用的具体地址列表，能在新版本上自动重新定位，实现持续有效性。
> - **范式转变：** Agent的经验适应性沉淀系统能自动抽取重复操作模式，将逆向从每次版本迭代的重复劳动，转变为方法论累积和Agent自动执行的新模式。
...
## 当我逆向 GarudaDefender 时,Hermes Agent 在逆向我
...
![Garuda Defender Hook Detected 弹窗](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/dd949f1b6718966a.png)
...
## 3\. libcrackme.so 中的 wrapper 与 resolver
...
![sub_61482C 调用栈截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/65f8e58ea93a4d0b.png)

### 18.5 和 JNI startActivity 的闭环
...
![SystemAlert Hook Detected 弹窗](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/dd949f1b6718966a.png)
...
![绕过成功截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/c8cbd099aa71b2d2.png)

`ok2.png` 是进一步绕过 root 检测、Frida 检测等环境检测后的主页面截图，用来支撑前面的 root/root-hide 检测返回值修正和其它环境检测绕过点最终生效：

![root 和 Frida 等环境检测绕过后的截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/9c093591408aae9e.png)

`001.png` 是未绕过时的弹窗截图，标题为 `Hook Detected!`，正文与 native 层 `sub_61482C` 打印到的文案一致：

![Hook Detected 弹窗截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/dd949f1b6718966a.png)

`stack.png` 是 `0x61482C / sub_61482C` 函数的堆栈截图。它支撑了两个结论：hook `sub_61482C` 后可以直接看到告警文案，并且可以通过 native backtrace 反推出上游调用链：

![sub_61482C stack](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/65f8e58ea93a4d0b.png)

`str.png` 是调用字符串解密/还原函数的代码片段截图，用来辅助说明这类检测特征字符串会经过运行时字符串处理路径：

![字符串解密调用代码片段](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/df15ec2d727358a0.png)

`thread_template.png` 是 `pthread_create` 创建线程后进入的第一层线程模板函数截图。该模板函数内部通过 `LDR X8, [X8,#0x10]` / `BLR X8` 从任务对象中取出真实任务入口：

![线程模板函数截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/734f1d7247d49dc6.png)

`0x911970.png` 是线程模板函数动态跳转到的关键任务入口截图。 `0x911970` 会继续调用 `sub_81DDF0` ，进入环境检测任务调度逻辑：

![0x911970 任务入口截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/e99e3f3a8c812d15.png)

`sub_81DDF0.png` 是 `sub_81DDF0` 中初始化 14 个环境检测任务的代码截图。这里能看到检测函数被成对写入任务表，随后通过 `sub_867F80(v166, 14)` 注册：

![sub_81DDF0 任务表截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/6fd516c60398d164.png)
...
![apatch root 框架字符串截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/0e9722421cfd63b4.png)

`root.png` 是未绕过 root 检测时 App 主页面显示 root 异常的截图：

![root 检测异常截图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/e4e65ce4fb0739b4.png)

`sub_6CEE14_01.png` 是 `sub_6CEE14` 这一组检测点的第一张分析图：

![sub_6CEE14 检测点图 1](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/2fb6a949f67e1f9d.png)

`sub_6CEE14_02.png` 是 `sub_6CEE14` 这一组检测点的第二张分析图：

![sub_6CEE14 检测点图 2](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/d0b00d4ef08864fa.png)

`sub_6CEE14_03.png` 是 `sub_6CEE14` 这一组检测点的第三张分析图：

![sub_6CEE14 检测点图 3](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/9693767d3024b802.png)

`sub_6CEE14_04.png` 是 `sub_6CEE14` 这一组检测点的第四张分析图：

![sub_6CEE14 检测点图 4](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/bdf20a7e3b3e24b7.png)

`sub_6CEE14_05.png` 是 `sub_6CEE14` 这一组检测点的第五张分析图：

![sub_6CEE14 检测点图 5](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/ff52b448eb0b12a9.png)
...
![sub_AEB528 检测点图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/d5a967ba6649aa8f.png)
...
![sub_AC7A90 检测点图](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/b24d3a8f1ff42296.png)
...
[#逆向分析](https://bbs.kanxue.com/forum-161-1-118.htm) [#NDK分析](https://bbs.kanxue.com/forum-161-1-119.htm) [#混淆加固](https://bbs.kanxue.com/forum-161-1-121.htm) [#脱壳反混淆](https://bbs.kanxue.com/forum-161-1-122.htm) [#HOOK注入](https://bbs.kanxue.com/forum-161-1-125.htm)
...
> **huangjw · 6 楼**
> 
> ![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/6bfc8df85281b342.png)
...
> **学计算机睡觉 · 10 楼**
> 
> 大佬牛逼！最想要的还是那份有价值的skill ![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/84e892700da67e86.png)

> **院士 · 11 楼**
> 
> 先收藏，慢慢领会。 ![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/06/a69be03d1bf35029.png)
