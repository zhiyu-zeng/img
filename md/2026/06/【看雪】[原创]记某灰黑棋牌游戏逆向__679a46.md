...
clip_date: 2026-06-15T17:02:45+08:00
trace_id: b56186d1-fcf1-4cef-a749-d29c59563aed
content_hash: 840f9606960f17bc0ad344c8509e59fc3a4c8214ef53a1b0a80384d2617e5f8e
status: summarized
tags:
  - 看雪
series: null
ai_summary: 成功逆向了一款四层壳保护的Cocos2d-x棋牌游戏，还原了其HMAC-SHA1请求签名算法。
ai_summary_style: key-points
images_status:
  total: 8
  succeeded: 8
  failed_urls: []
notion_page_id: 38075244-d011-8164-9ceb-d6ddfb3b3230
---

> 💡 **AI 总结（key-points）**
>
> 成功逆向了一款四层壳保护的Cocos2d-x棋牌游戏，还原了其HMAC-SHA1请求签名算法。  
> 
> - **壳结构：** Java壳 → `librnbqzphi.so`（解密dex）→ `libcocos2dlua.so`（运行时解密映射子so）→ 子so（执行魔改LuaJIT字节码）。解密使用RC4+zlib，密钥由类名凯撒位移生成。  
> - **关键突破：** 通过hook自定义linker在relocation前dump内存，并修复ELF段信息，得到可分析的子so（45.5MB）。同时hook `lj_str_new` 函数，在LuaJIT字符串驻留时抓取到签名素材明文。  
> - **opcode还原：** 在子so中定位LuaJIT分发循环和初始化函数，逆出魔改opcode→handler映射表（105个opcode）。通过频率分析和已知小函数锚点，确定磁盘字节码的魔改编号。  
> - **反编译方案：** 修改ljd反编译器的`_MAP`查找表，使魔改字节码能正确解析，同时保留指令类中的标准opcode值以避免范围判断错误。批量反编译成功845/850个.luajit文件。  
> - **签名公式：** `sign = HMAC_SHA1(key, material)`，其中`material = "{timestamp}|{按key升序的query参数}|{timestamp}|{key}|{version}|{urlpath}"`。密钥（如`zSWY1et3fPeY9PfY#^pvg5tBGzSjZRi2`）隐写于特定图片文件中。
