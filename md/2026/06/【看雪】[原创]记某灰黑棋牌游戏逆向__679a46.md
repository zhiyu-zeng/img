...
clip_date: 2026-06-15T15:16:54+08:00
trace_id: a1a0bfcd-827f-42e0-a5ce-ca602340cf96
content_hash: daa2878fd20f9b4f94aeb34e7fa6611b9f18d713cc3562fcb304e98d8d710abb
status: summarized
tags:
  - 看雪
series: null
ai_summary: 通过动态调试与静态分析相结合，成功破解了一个经过多层加壳、Lua脚本加密及字节码魔改的棋牌APP的HMAC-SHA1签名算法。
ai_summary_style: key-points
images_status:
  total: 4
  succeeded: 4
  failed_urls: []
notion_page_id: 38075244-d011-816d-a86f-c368782b5f0b
---

> 💡 **AI 总结（key-points）**
>
> 通过动态调试与静态分析相结合，成功破解了一个经过多层加壳、Lua脚本加密及字节码魔改的棋牌APP的HMAC-SHA1签名算法。
> 
> - **应用保护机制：** APP采用四层RC4+zlib壳，子so内存加载且自定义linker，Lua脚本被XXTEA加密，LuaJIT字节码的opcode编号被全面重排。
> - **系统破解思路：** 整体流程为：脱壳获取dex和子so -> Hook获取解密后的明文Lua脚本 -> 在IDA中逆向分析魔改LuaJIT，还原opcode映射表 -> 修复反编译器，反编译Lua脚本。
> - **关键难点突破：** 针对不落地的子so，通过Hook其自定义linker，在relocation前时机精准dump内存段并修复ELF头；针对魔改LuaJIT，通过识别解释器分发循环结构，静态分析出opcode分发表初始化逻辑，还原了映射关系。
> - **签名还原过程：** 静态反编译得出签名函数结构，但因slot复用导致无法静态推导精确素材。转而通过Hook LuaJIT字符串驻留函数，动态抓取到HMAC-SHA1的输入，验证了其为标准算法。
> - **密钥来源：** 签名所需HMAC密钥和相关配置（appid等）通过图片隐写藏在资源文件中，在同一build内稳定。
