...
clip_date: 2026-08-04T10:42:15+08:00
trace_id: b1cf0313-f08a-47cb-8d16-5e70603d1c2e
content_hash: e953c36a1e2ce356f2b182cfc92053e78ea56cd7c10fa653c42c30f578f3cfeb
status: synced
tags:
  - Android逆向
  - 协议分析
...
ai_summary: 学习通APP登录与课程接口的加密参数均可逆向还原，验证码enc为MD5，logininfo为AES+Base64，token等部分硬编码，cxcid采用RSA解密，可实现自动化请求。
...
images_status:
  ...
  failed_urls:
    ...
    - 29.png
    - 30.png
notion_page_id: 3b275244-d011-814e-8288-e89a8500413e
ioc:
  cves: []
  cwes: []
  hashes:
...
> 💡 **AI 总结（key-points）**
>
> 学习通APP登录与课程接口的加密参数均可逆向还原，验证码enc为MD5，logininfo为AES+Base64，token等部分硬编码，cxcid采用RSA解密，可实现自动化请求。
> 
> - **验证码接口enc：** enc由`MD5(手机号 + "jsDyctOCnay7uotq" + 毫秒时间戳)`生成，通过搜索路径`api/sendcaptcha`定位到Retrofit调用，校验逻辑位于`ie.j.c`。
> - **登录参数logininfo：** 将`{"uname":"手机号","code":"验证码"}`以AES-ECB/PKCS5Padding加密，密钥为`z4ok6lu^oWp4_AES`（来自Keys.KEY_26），结果再经标准Base64编码，可通过hook验证明文。
> - **静态Token与inf_enc：** `token`、`_key`等直接硬编码在`Keys.java`中（如`token = "4faa8662c59590c6f43ae9fe5b002b42"`）；`inf_enc`为所有查询参数按字母顺序拼接后计算MD5。
> - **cxcid解密：** 参数值由native函数`decryptDeviceInfo`计算，加载`libsecuritylib.so`，实为标准Base64解码后以RSA公钥解密（公钥可提取），返回的明文包含设备相关信息。
> - **cryptUserAgent与web_request_sign：** `cryptUserAgent`是在UA前添加固定盐值的MD5签名；`web_request_sign`将JSON键值排序拼接后用内置RSA私钥签名，私钥可从so中dump。

## 0x00 验证码&登录

先抓包

![1](⚠️ https://yuuki.cool/posts/xvexitong/1.png)

可以看到请求头里没啥，看看请求体

![2](⚠️ https://yuuki.cool/posts/xvexitong/2.png)
...
![3](⚠️ https://yuuki.cool/posts/xvexitong/3.png)

只有这一处调用，而且app字符串没看到有加密的，大概率就是这里了。hook验证之后可以发现确实是这里。看一下 `f` 的引用，发现只有一处，可以定位到这里

![4](⚠️ https://yuuki.cool/posts/xvexitong/4.png)
...
![5](⚠️ https://yuuki.cool/posts/xvexitong/5.png)
...
![6](⚠️ https://yuuki.cool/posts/xvexitong/6.png)
...
![7](⚠️ https://yuuki.cool/posts/xvexitong/7.png)

![8](⚠️ https://yuuki.cool/posts/xvexitong/8.png)
...
![9](⚠️ https://yuuki.cool/posts/xvexitong/9.png)

![10](⚠️ https://yuuki.cool/posts/xvexitong/10.png)

![11](⚠️ https://yuuki.cool/posts/xvexitong/11.png)
...
![12](⚠️ https://yuuki.cool/posts/xvexitong/12.png)
...
![13](⚠️ https://yuuki.cool/posts/xvexitong/13.png)
...
![14](⚠️ https://yuuki.cool/posts/xvexitong/14.png)

果然是几个字段都出现了，这里跟一下 `token` 字段，发现他是在函数 `j` 里赋值的，这里看一下 `j` 函数的引用

![15](⚠️ https://yuuki.cool/posts/xvexitong/15.png)

添加了一个 `pair` 对象，token的值是 `pair.first` ，这里也关注一下 `pair.second` ，它的 `_key` 的值

![16](⚠️ https://yuuki.cool/posts/xvexitong/16.png)
...
![17](⚠️ https://yuuki.cool/posts/xvexitong/17.png)

`q.b(sb2.toString())` ，去看看 `q.b()` 吧

![18](⚠️ https://yuuki.cool/posts/xvexitong/18.png)
...
![19](⚠️ https://yuuki.cool/posts/xvexitong/19.png)

验证一下

![20](⚠️ https://yuuki.cool/posts/xvexitong/20.png)
...
![21](⚠️ https://yuuki.cool/posts/xvexitong/21.png)

静态注册，也没混淆，函数名可以一眼看出来是 `Base64+RSA`

![22](⚠️ https://yuuki.cool/posts/xvexitong/22.png)
...
![23](⚠️ https://yuuki.cool/posts/xvexitong/23.png)

![24](⚠️ https://yuuki.cool/posts/xvexitong/24.png)
...
![25](⚠️ https://yuuki.cool/posts/xvexitong/25.png)

确实是标准的 `Base64` ，直接用标准RSA算法解密一下看看，确实输出了预期的结果

![26](⚠️ https://yuuki.cool/posts/xvexitong/26.png)
...
![27](⚠️ https://yuuki.cool/posts/xvexitong/27.png)

`iv` 没改，看下MD5签名的参数吧

![28](⚠️ https://yuuki.cool/posts/xvexitong/28.png)
...
![29](⚠️ https://yuuki.cool/posts/xvexitong/29.png)

![30](⚠️ https://yuuki.cool/posts/xvexitong/30.png)
