---
title: 【先知】记一次 HIS 系统垂直越权挖掘：从弱口令登录到垂直越权导致全量信息泄露
source: https://xz.aliyun.com/news/92669
source_host: xz.aliyun.com
clip_date: 2026-08-11T15:28:28+08:00
trace_id: b198ec59-314f-43ba-8f1d-8e351f93027a
content_hash: 1e1fd6f720df568be7d92e47501a6201dacf0e075e914943a2684246d2027c15
status: synced
tags:
  - 先知
  - 风控对抗
...
ai_summary: 一次针对医院 HIS 系统的攻防测试表明：自研令牌签名密钥可逆向，低权限账号可伪造令牌调用无 RBAC 校验的管理接口，导致全量用户数据泄露。
ai_summary_style: key-points
images_status:
  total: 18
  succeeded: 18
  failed_urls: []
notion_page_id: 3b975244-d011-81ee-9cb4-d0b4c683f8e6
ioc:
  cves: []
  cwes: []
  hashes:
    - 61cc39fcc932a97d4ec0df6138746a50
  domains: []
  tools: []
  techniques: []
---
...
> 💡 **AI 总结（key-points）**
>
> 一次针对医院 HIS 系统的攻防测试表明：自研令牌签名密钥可逆向，低权限账号可伪造令牌调用无 RBAC 校验的管理接口，导致全量用户数据泄露。
> 
> - **入口突破：** 登录框有密码加密和失败次数限制，用固定密码 `1qaz@WSX` 爆破用户名，命中普通血透医生账号 `dengli`，登录后仅见1个应用、角色为血透医生。
> - **接口提取：** 主 JS 为 webpack 打包，用正则脚本提取出 160 个接口（用户/患者/账号/角色/菜单等）；未带令牌请求全部报鉴权失败，带登录态却提示“Token校验失败”，发现 Authorization 头每次请求动态变化。
> - **令牌逆向：** 令牌为 5 段 Base64（`.`分隔）：`UUID.时间戳.随机串.MD5签名.用户信息`；在 JS 中找到签名函数 `Em`，还原算法为 `MD5(token+时间戳+随机串+参数串+secret+用户信息串).upper()`，其中 `secret`、`token` 均存于本地存储，可直接伪造任意请求签名。
> - **垂直越权：** 用低权限令牌伪造 `POST /msun-middle-base-staff/user/findIncludeStaffByPage` 请求，成功分页获取大量医生/用户信息；其他管理接口同样可越权访问，改 `pageSize` 即可返回全量数据。
> - **根因与坑：** 管理接口只验登录态、不验接口 RBAC 权限；`secret` 明文下发前端；详情接口无对象级授权（IDOR）。签名绑定请求体，换 body 需重新生成；签名时间戳约2-3分钟过期，需实时生成；直接改令牌中的用户信息不可行，后端会校验用户绑定字段。
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/cdfe03b31fe87389.png)

雪瞳收集了一些 **ID密钥**，泄露的 **TextIn secret** 能利用，先捡个低危恰烂分。

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/e4d2a647b3ee3285.png)

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/364ad5568fb48657.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/f78aac6b7ea84bda.png)

系统登录页尝试弱口令，发现密码进行了加密并且限制了登录失败次数，针对这种场景，可以采用固定密码爆破用户名的方式突破。

像国内的很多OA、医疗系统、私有邮箱、协同办公以及一些内网系统很喜欢用姓名拼音或者工号来作为登录用户名。

固定密码 **1qaz@WSX**，爆破用户名，这里运气不错爆出一个用户名为dengli的账户

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/df2d82b6ce7f2fb1.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/c2c9b1702e5e625a.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/cd21c5d1cf39fc11.png)

### 3\. AI 辅助越权访问

接口多、参数未知，让 AI 根据接口语义猜测参数（userId、pageNum、hospitalId 等），脚本自动拼 URL 批量发包， **接口均有鉴权**。

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/d9fd156a9f3f1d6a.png)

那么带着登录态的令牌呢？

报错全变成了 **Token校验失败**

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/9c070fca33cd74b2.png)

回头翻了翻burp的历史，发现每个数据包的令牌竟然是 **动态变化** 的？

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/1c94406ab3aa3f8a.png)

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/5c5ec4cf2aa8de5f.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/8655987bef63fe32.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/c9819b02009e9718.png)
...
这里需要的 **secret**、 **token** 全都在本地存储，用户信息串使用当前登录用户的信息。

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/42f4a51fe9b22646.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/3d4c8bb272a8296f.png)

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/8379677954137a68.png)
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/4757dfac9e72ef5f.png)

使用伪造的令牌访问接口，这里注意POST的 **body** 要和签名中的一致。

![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/b9bcd277a13b4767.png)
...
这个系统"看似鉴权完善"（第一轮接口测试全被拦），实则 **签名密钥明文下发 + 管理接口无权限校验** 两处设计缺陷叠加，导致：
