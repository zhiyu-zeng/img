---
title: 【先知】MariaDB 远程代码执行漏洞分析复现
source: https://xz.aliyun.com/news/92636
source_host: xz.aliyun.com
clip_date: 2026-08-06T16:36:11+08:00
trace_id: bbf97b1d-73f5-4bea-b07e-27ea31a6277d
content_hash: 508a57066954d7cc447c5262269e01094e2a885f2c6ca1569f58b4896d9913ae
status: synced
tags:
  - 先知
  - 漏洞分析
...
ai_summary: MariaDB 13.0.1 存在由"GRANT PROXY 提权 (F-09)"与"SYS_REFCURSOR 释放后使用 (F-05)"两个 0day 串联组成的远程代码执行链，低权限 USAGE 账户仅凭纯 SQL 即可在服务器端以 mysql 身份执行任意系统命令，CVSS 3.1 评分为 9.8（Critical）。
ai_summary_style: key-points
images_status:
  total: 1
  succeeded: 1
  failed_urls: []
notion_page_id: 3b475244-d011-81f5-be79-df32e098bbef
ioc:
  cves: []
  cwes: []
  hashes:
    - ef34af04bda12e6c85395328af78d562176c34fb29ae52063a4eb0d68fa7b3e9
  domains: []
  tools: []
  techniques: []
---
...
> 💡 **AI 总结（key-points）**
>
> MariaDB 13.0.1 存在由"GRANT PROXY 提权 (F-09)"与"SYS_REFCURSOR 释放后使用 (F-05)"两个 0day 串联组成的远程代码执行链，低权限 USAGE 账户仅凭纯 SQL 即可在服务器端以 mysql 身份执行任意系统命令，CVSS 3.1 评分为 9.8（Critical）。
> 
> - **漏洞链组成：** F-09（所有已发布版本受影响）可利用 `GRANT PROXY ... IDENTIFIED VIA ''` 绕过权限检查，将任意已认证用户提升为 root；F-05 利用 `sp_cursor.cc` 中 `sp_cursor_array::get_cursor_by_ref()` 返回内部动态数组指针、游标打开时数组扩容触发 `my_realloc` 后指针悬挂，导致虚函数调用被劫持；两个漏洞截至 2026-08-03 上游仍未修复。
> - **利用前置条件：** 攻击者需拥有任意低权限数据库账户、可 TCP 访问 3306 端口、`secure_file_priv` 未设置（官方 Docker 镜像默认）、glibc + x86_64 环境、`/proc` 可读；`max_allowed_packet` 调大需求可借 F-09 提权后自动满足。容器环境默认 `SYS_PTRACE` capability 仅作为辅助条件。
> - **攻击链流程：** 先用一句 SQL 将 root 密码改为空并提权；再用 `LOAD DATA INFILE '/proc/self/maps'` 读取内存布局实现 ASLR 绕过；随后分配 128 MiB 用户变量作为假 vtable 与 JOP 链缓冲区，通过 SQL 端 maps 差分和 glibc mmap 槽位复用保证地址稳定；最后调用 `CALL uaf5` 触发 UAF，经 `call *0x100(%rax)` 与 `mov rdi,[rax+0xa8]; call [rax+0xa0]` 跳转至 `system()` 执行 `sh -c` 命令。
> - **复现验证：** 使用官方 `mariadb@sha256:ef34...`（13.0.1-ubu2604）镜像与低权限用户 `lowpriv`，运行 `exploit_pure_sql.py` 后 `/tmp/pwned` 内容为 `uid=999(mysql) gid=999(mysql)`，确认 RCE 生效；执行后 mariadbd 崩溃属预期。
> - **修复建议：** 短期缓解可设置 `secure_file_priv` 为空或安全目录、限制 `max_allowed_packet`（如 16M）、限制 `max_open_cursors`、移除容器 `SYS_PTRACE` capability、网络隔离；根本修复需合入 MDEV-40470 补丁（`dbd60d0ad8d`）并重构 `sp_cursor_array` 改用索引或稳定容器（如 `std::deque`）避免悬挂指针。
...
| ASLR 开启 | 实际上 **ASLR 开启反而是利用条件之一**，因为 `/proc/self/maps` 恰好能泄露随机化后的地址。如果 ASLR 关闭，利用更简单（固定地址） | Linux 默认开启 |
...
![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/e77a669eef0efa70.png)
...
### 第四阶段：F-05 SYS_REFCURSOR Use-After-Free
...
4.  **禁用 SYS_PTRACE capability** （容器环境），移除 `cap_add: SYS_PTRACE`
