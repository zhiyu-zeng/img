---
title: 自定义Linker与SO加固 - Yuuki
source: https://yuuki.cool/posts/customlinker/
source_host: yuuki.cool
clip_date: 2026-08-07T19:13:59+08:00
trace_id: e15d1def-777d-4370-965f-10b0fd6e838d
content_hash: 09e1aaa0b0841a138a9159a6880451ff295b9694fd534722fc90022d07038114
status: synced
tags:
  - Android逆向
  - 脱壳与加固
series: null
feed_source: null
ai_summary: 360加固通过在主SO中嵌壳SO实现保护；文章分析Android Linker源码后，实现自定义Linker，并演示了将RC4加密主SO藏入WebP图片的简化SO加固方案。
ai_summary_style: key-points
images_status:
  total: 1
  succeeded: 1
  failed_urls: []
notion_page_id: 3b575244-d011-81e6-82b0-c132a5e45854
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 360加固通过在主SO中嵌壳SO实现保护；文章分析Android Linker源码后，实现自定义Linker，并演示了将RC4加密主SO藏入WebP图片的简化SO加固方案。
> 
> - **加固原理：** 360加固方案在主SO内嵌入另一个SO作为壳，由原生Linker加载壳后解密并加载主SO；简化版将RC4加密的主SO追加到`res/mipmap/ic_launcher.webp`图片后，以`Yuuki`为魔数标识。
> - **脱壳关键点：** `DT_INIT`和`DT_INIT_ARRAY`是SO加载后最早执行的初始化入口，常作为加固脱壳点；重定位依赖`mmap`，因此可在`/proc/pid/maps`中观察加载痕迹。
> - **IDA干扰技巧：** Android Linker只读Program Header且只解析第一个`PT_DYNAMIC`段，而IDA依赖Section Header，因此“抹头”（破坏Section Header）和添加多个动态节可迷惑IDA但不影响运行。
> - **加载流程：** Linker核心流程为`find_loaded_library`→`load_library`→`soinfo_link_image`（解析动态节、递归加载`DT_NEEDED`、执行重定位）→`CallConstructors`。
> - **自定义Linker架构：** 精简实现包含`ElfLoader`、`ElfReader`、`MemoryManager`、`SoinfoManager`、`Relocator`、`Utils`等组件；解密流程为Java层从APK提取图片，Native层在`ElfReader::HandleFileType`中检测魔数并RC4解密，再由自定义Linker透明加载执行。
...
![Relationship](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/01898f91e9bf04f4.webp)
