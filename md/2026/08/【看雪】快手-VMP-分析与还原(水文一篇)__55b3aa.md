---
title: 【看雪】快手 VMP 分析与还原(水文一篇)
source: https://bbs.kanxue.com/thread-292535.htm
source_host: bbs.kanxue.com
clip_date: 2026-08-15T22:11:38+08:00
trace_id: 152503bd-bfec-4612-b409-624571a37a2f
content_hash: c0cc93d942d86713e18c5094370401670e8d24df7c7cf4669f9ede43e096b9d4
status: synced
tags:
  - 看雪
  - Android逆向
...
ai_summary: 针对快手 __NS_xfalcon VMP，提出以 trace + 地址热点定位、last_writer 切 handler、VEX IR/DFG 自动还原的方法，全量 25558 段零错误还原。
ai_summary_style: key-points
images_status:
  total: 0
  succeeded: 0
  failed_urls: []
notion_page_id: 3bd75244-d011-8191-ad9c-f7f54a106b81
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> 针对快手 __NS_xfalcon VMP，提出以 trace + 地址热点定位、last_writer 切 handler、VEX IR/DFG 自动还原的方法，全量 25558 段零错误还原。
> 
> - **定位与切割：** 通过地址热度与 `br x8` 等寄存器跳转定位链式 VMP；维护 last_writer 表，识别 `add xR,x25,xR` 写出的寄存器再 `br` 即为分发点，可跨指令区分复合 handler，避免全局 bool 失效。
> - **还原管线：** handler 汇编逐指令经 pyvex 提升为 IRSB，按寄存器 offset 构建 DFG；锚点过滤保留 x21/sp/x23/x20 的 VM 状态符号，将 x26 读出的字节码操作数折叠为常量，再经常量传播、代数化简到不动点，输出短 IR。
> - **结果数据：** 全量 25558 段，sink 24321 行，可校验 24055 行全部正确（✓ 24055 / ✗ 0），266 条因 trace 未记录 load 值而无值；已支持 mov/add/and/MAX/align32/sext 等语义及 `// call @0x...` 标注。
> - **已知缺口：** 复合分配器 handler（TPIDR_EL0 取 TLS、free-list 弹出）指针解引用超出锚点；运行时计算偏移的 ctx 写会丢失符号形式（如 `ctx[vreg[A]+B]`）；无值 sink 无法对账。
...
结果发现 angr 直接可以用 P-code(angr 维护的 pypcode)，然后打算直接 keystone+angr 了，但是发现 angr 对 P-code 的支持不如 VEX IR，测了 10 个 handler，每个 handler 进行 10 次 hash 计算，确定是稳定的以后就直接换 VEX IR 了，最后发现 angr 可以直接吃明文汇编，底层自动转化的所以直接 ks 也不需要了然后我们就得到了 `angr的project、state` ，接下来就到了算法部分了(快手这个 handler 写的好烂啊，几乎找不到合适讲解的)，下面是我把上面的逻辑让 AI 生成的图
...
| `x21 + idx` | `VM_REG[slot]` |
| `sp + imm` | `VM_STACK[off]` |
| `x23 + off` / `x20 + off` | `CTX[off]` / `CTX_S[off]` |
| 从栈 load 出的指针再解引用 | `*STACK_PTR` |
| `add R, x21, X` 作为值被存 | `&vreg[...]` （绝对地址对象） |
