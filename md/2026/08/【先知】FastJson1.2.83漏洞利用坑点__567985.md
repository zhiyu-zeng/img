...
clip_date: 2026-08-13T12:14:22+08:00
trace_id: 032c9c32-09d5-4a96-8227-527297d1a996
content_hash: 6dc68ff58853e48101f42bea83ce44b08d62276207857a8dfe27519bad45a584
status: synced
tags:
  - 先知
  - 漏洞分析
...
series: null
feed_source: null
ai_summary: Fastjson 1.2.83远程代码执行并非Spring Boot+该版本即可触发，真正利用依赖Fastjson绑定LaunchedURLClassLoader、构造特殊包名恶意类，并需处理JDK9+远程类加载限制。
ai_summary_style: key-points
images_status:
  total: 2
  succeeded: 2
  failed_urls: []
notion_page_id: 3bb75244-d011-8189-8a8d-fe760e796706
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> Fastjson 1.2.83远程代码执行并非Spring Boot+该版本即可触发，真正利用依赖Fastjson绑定LaunchedURLClassLoader、构造特殊包名恶意类，并需处理JDK9+远程类加载限制。
> 
> - **核心坑点：** 仅搭建Spring Boot FatJar+1.2.83+JDK8默认无法RCE，公开EXP必须通过`ParserConfig.getGlobalInstance().setDefaultClassLoader(...)`强制绑定LaunchedURLClassLoader；否则Fastjson会走TomcatEmbeddedWebappClassLoader，只能触发请求或SSRF。
> - **EXP构造：** 恶意类package必须写成`jar:http:..xxx.xxx!;`，使Fastjson将类型名映射为远程资源路径；若用正常`com.test`包名会在本地ClassPath查找并报ClassNotFoundException，且普通`javac`无法编译含`:`、`/`、`!`的非法包名，需用ASM/Javassist生成字节码。
> - **@JSONType注解：** 恶意类必须标注`@JSONType`，Fastjson加载目标类后会检查该注解并据此进入后续流程；实际RCE的执行点不是注解本身，而是类的`static{}`（JVM `<clinit>`初始化）。
> - **JDK9+绕过：** 直接`jar:http://...`远程加载受限，需先发送第一请求让目标JVM下载jar并保持临时文件FD打开，再遍历`/proc/self/fd/N`构造`jar:file:/proc/self/fd/N!/POC`，把远程jar语义转为本地file再完成加载与初始化。
> - **完整利用模型：** 实际链路由第一次HTTP jar请求、第二次FD遍历请求组成，本质是"Fastjson+特殊ClassLoader环境+恶意Class构造+JVM类初始化"才能RCE，不能简单等同为"Fastjson 1.2.83=直接RCE"。
