---
title: 【GitHub】stackplz/README.md at dev
source: https://github.com/SeeFlowerX/stackplz/blob/dev/README.md
source_host: github.com
clip_date: 2026-08-14T17:26:08+08:00
trace_id: e4030f3e-9c74-4b26-a24c-e932ddeeec57
content_hash: 124bbc42f71ba5aac0c2190dfb5420d26fcdfb075717e451c9fe74890dd85329
status: synced
tags:
  - GitHub
  - Android逆向
...
series: null
feed_source: null
ai_summary: stackplz 是一款基于 eBPF 的 Android 堆栈追踪工具，支持 syscall/uprobe hook 和硬件断点，可打印参数、调用栈、寄存器，并支持进程/线程过滤及 Frida 联动。
ai_summary_style: key-points
images_status:
  total: 3
  succeeded: 3
  failed_urls: []
notion_page_id: 3bc75244-d011-81b6-824b-e35da7ce3a1f
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> stackplz 是一款基于 eBPF 的 Android 堆栈追踪工具，支持 syscall/uprobe hook 和硬件断点，可打印参数、调用栈、寄存器，并支持进程/线程过滤及 Frida 联动。
> 
> - **功能范围：** 支持 arm64 syscall trace、64 位用户态动态库 uprobe hook、硬件断点；可输出参数（含结构体）、调用栈、寄存器，并可通过 `--rpc` 做 Frida 联动远程下断。
> - **环境要求：** 需 root 且内核 5.10+（`uname -r` 查看）；4.1x 内核开启 `CONFIG_HAVE_HW_BREAKPOINT` 时也可用硬件断点；支持 arm 开发板刷安卓镜像、Docker+ReDroid、Apple M 系列模拟器等环境。
> - **过滤能力：** 支持按包名/uid/pid/tid/线程名黑白名单过滤，进程分组有 root/system/shell/app/iso；syscall 可按分组追踪（如 `%file`、`%net`）；参数可设规则：`w:` 字符串白名单、`b:` 字符串黑名单、`bx:` 16 进制 buffer 匹配（最多 8 字节）、`eq:` 寄存器值相等匹配。
> - **使用方式：** 将预编译二进制 push 到 `/data/local/tmp` 并加执行权限，v3.0.1 前需先 `./stackplz --prepare` 释放库文件；hook 动态库用 `-w` 指定符号/偏移并配合 `-l/--lib`，硬件断点用 `--brk` 结合 `--pid` 或 `--brk-lib` 指定地址/相对偏移。
> - **注意事项：** 默认屏蔽渲染/后台高频线程，可用 `--full-tname` 解除；hook 符号可能被 IFUNC 解析成实际实现（如 `strchr` 对应 `__strchr_aarch64`）；syscall 返回值是 errno，与 libc 返回值不一定一致；perf buffer 满时用 `-b` 增大缓冲（默认 8M，建议不超过 32M）；`--jstack` 需搭配 `--kill SIGSTOP` 使用。
...
-   client frida脚本参考 [frida_hw_brk.js](https://github.com/SeeFlowerX/stackplz/blob/dev/frida_hw_brk.js)
...
-   `-c/--config` 配置文件模式
    -   配置文件具体使用方式请查看 [配置文件文档](https://github.com/SeeFlowerX/stackplz/blob/dev/docs/CONFIG.md)
...
[![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/e36e5ad6ae163168.png)](https://github.com/SeeFlowerX/stackplz/blob/dev/images/Snipaste_2023-07-22_21-17-17.png)
...
[![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/c731226a69dbf2ef.png)](https://github.com/SeeFlowerX/stackplz/blob/dev/images/Snipaste_2023-07-22_21-21-33.png)
...
[![](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/089f08636f2dfec4.png)](https://github.com/SeeFlowerX/stackplz/blob/dev/images/Snipaste_2024-03-04_21-51-12.png)
...
-   408 -> std::string soname\_;
-   432 -> std::string realpath\_;
-   152 -> linker_ctor_function_t\* init_array\_;
-   160 -> size_t init_array_count\_;
...
具体分组情况请查看 [Parse_SyscallNames](https://github.com/SeeFlowerX/stackplz/blob/dev/user/config/config_module.go)
...
可参考 [workflow](https://github.com/SeeFlowerX/stackplz/blob/dev/.github/workflows/build.yml) 或查看 [编译文档](https://github.com/SeeFlowerX/stackplz/blob/dev/docs/BUILD.md) ：
