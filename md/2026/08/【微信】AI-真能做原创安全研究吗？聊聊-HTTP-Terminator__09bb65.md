---
title: 【微信】AI 真能做原创安全研究吗？聊聊 HTTP Terminator
source: https://mp.weixin.qq.com/s/QqdZ1M8a4PZHu6AIL8-ROg
source_host: mp.weixin.qq.com
clip_date: 2026-08-13T10:30:45+08:00
trace_id: 432e664e-aad5-44ee-a600-21cb3d458159
content_hash: a3e5e70cce32f3f5fe7b8d09da18b0a3f1870efa7f8e3a4b1b14ff7258d6643b
status: synced
tags:
  - 微信
  - AI应用
...
ai_summary: AI 能做出原创安全研究——HTTP Terminator 自主发现全新攻击类别并攻破真实网站，但最关键的级联发现在人机交替中才被激活，人的判断是系统的放大器。
ai_summary_style: key-points
images_status:
  total: 5
  succeeded: 5
  failed_urls: []
notion_page_id: 3bb75244-d011-8112-856f-ed771bde4c50
ioc: null
---

> 💡 **AI 总结（key-points）**
>
> AI 能做出原创安全研究——HTTP Terminator 自主发现全新攻击类别并攻破真实网站，但最关键的级联发现在人机交替中才被激活，人的判断是系统的放大器。
> 
> - **系统规模：** HTTP Terminator 将 138 个 RFC 切成 1.5 万个微碎片，生成 3 万个去重去同步向量；用 Burp 插件 + SQLite 在 EC2 上以 2000 线程扫描 3 万网站，每域名每秒限 1 次请求，最终筛出约 700 个脆弱目标。
> 
> - **原创触发器：** 多个新向量被验证有效，如 `Transfer-Encoding: gzip` 配 `HTTP/1.0` 在 F5 Big-IP 上导致 CL.0 去同步，攻破一个美国政府网站和机场系统，访问到含航班、乘客、行李信息的员工面板；`multipart/byteranges` 变体影响超 200 个网站（含一家美国银行）。
> 
> - **武器化技巧：** 代理第一次攻击时表现新手化（误把流水线当漏洞、执着连接复用），通过把 MCP 改名为“Turbo Simulator”并隐藏 `Connection: close` 头解决；悬垂字节（故意少发一个字节的残缺请求）消除了响应队列投毒的竞态，大幅提升稳定性。
> 
> - **级联发现：** 评估 bug 意外触发内存泄漏，异常检测发现“HTTP/0.9 双压缩响应”等新现象；人手与 AI 交替验证后提出“共享解析器混淆”——利用前后端解析器逻辑差异而非长度分歧，可明确区分 F5 前后端状态机，这是一个人类或 AI 单独都无法发现的新攻击类别。
> 
> - **方法论经验：** 全 LLM 驱动难以迭代，改用“AI 编写+确定性代码把关”实现零误报；生成假设时需显式排除低价值方向、给具体问题，避免上下文污染；最值钱的经验是尽早做评估原语、控制数据质量，并让人负责级联发现这一最终创造性环节。

**幻泉之洲** *2026年8月13日 10:12*
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/6d3c25a9a2839501.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/11726a2241628fc2.png)

![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/f85d15112eb9b5b8.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/527934320b8df286.png)
...
![图片](https://cdn.jsdelivr.net/gh/zhiyu-zeng/img@main/img/2026/08/44be0ab6b4ac04dd.png)
